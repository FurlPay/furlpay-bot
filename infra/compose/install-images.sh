...
DOWNLOAD_BASE="${FURLPAY_DOWNLOAD_BASE:-https://raw.githubusercontent.com/FurlPay/furlpay-bot/main/infra/compose}"
...
case "$DOWNLOAD_BASE" in
  https://*) ;;
  *)
    echo "FurlPay Bot setup failed: FURLPAY_DOWNLOAD_BASE must use https." >&2
    exit 1
    ;;
esac
...
prepare_only=false
skip_existing=false
if [[ "${FURLPAY_DOWNLOAD_SKIP_EXISTING:-}" == "1" ]]; then
  skip_existing=true
fi
...
fail() {
  echo "FurlPay Bot setup failed: $*" >&2
  exit 1
}
...
services:
  api:
    environment:
      _FURLPAY_VALIDATE_POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:?Set POSTGRES_PASSWORD in .env}
      _FURLPAY_VALIDATE_BETTER_AUTH_SECRET: ${BETTER_AUTH_SECRET:?Set BETTER_AUTH_SECRET in .env}
      _FURLPAY_VALIDATE_ENCRYPTION_KEY: ${ENCRYPTION_KEY:?Set ENCRYPTION_KEY in .env}
      _FURLPAY_VALIDATE_SCREEN_PROXY_SECRET: ${SCREEN_PROXY_SECRET:?Set SCREEN_PROXY_SECRET in .env}
      _FURLPAY_VALIDATE_SANDBOX_SUPERVISOR_TOKEN: ${SANDBOX_SUPERVISOR_TOKEN:?Set SANDBOX_SUPERVISOR_TOKEN in .env}
YAML
  then
...
if [[ "$prepare_only" == true ]]; then
  echo "FurlPay Bot files are ready. Edit .env, then run: bash install-images.sh"
  exit 0
fi
...
echo "FurlPay Bot is starting at http://127.0.0.1:5173"

...
"${compose[@]}" exec -T postgres sh -c \
  ...
  > "${SNAPSHOT_DIR}/furlpay-bot.dump"

"${compose[@]}" exec -T api tar -czf - -C /data . \
  > "${SNAPSHOT_DIR}/appdata.tgz"

"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/furlpay-bot.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/furlpay-bot.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified FurlPay Bot backup written to ${SNAPSHOT_DIR}"

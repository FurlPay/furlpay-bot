...
import type { ServerUpdateRun } from "@furlpay-bot/contracts";
...
const app = createUpdaterApp(
  resolveUpdaterConfig({
    FURLPAY_DEPLOY_DIR: "/furlpay-bot-updater-tests-no-such-directory",
    FURLPAY_UPDATER_TOKEN: token,
  }),
);
...
async function deployment(env = "FURLPAY_IMAGE_TAG=v1.0.0\nFURLPAY_IMAGE_TAG_PREVIOUS=v0.9.0\n") {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "furlpay-bot-updater-test-"));
  temporaryDirectories.push(deployDir);
  await writeFile(path.join(deployDir, ".env"), env);
  return {
    deployDir,
    config: resolveUpdaterConfig({ FURLPAY_DEPLOY_DIR: deployDir, FURLPAY_UPDATER_TOKEN: token }),
  };
}
...
describe("updater HTTP surface", () => {
  ...
  it("re-validates the branch at its own boundary", async () => {
    const response = await app.request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/FurlPay/furlpay-bot", branch: "--exec=id" }),
    });
  ...
  it("refuses a fork build when the deployment has no checkout to build from", async () => {
    ...
    const response = await createUpdaterApp(fixture.config).request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/someone/furlpay-bot", branch: "main" }),
    });
    expect(response.status).toBe(400);
    const payload = (await response.json()) as { error: string };
  ...
  it("refuses a rollback when no previous tag was recorded", async () => {
    const fixture = await deployment("FURLPAY_IMAGE_TAG=v1.0.0\n");
...
describe("updater orchestration", () => {
  it("serializes updates before their asynchronous preflight can race", async () => {
    ...
    const input = { repoUrl: "https://github.com/FurlPay/furlpay-bot", branch: "main" };
  ...
  it("restores the prior image after a recreate fails health checks", async () => {
    ...
    const response = await request(subject, "/apply", {
      repoUrl: "https://github.com/FurlPay/furlpay-bot",
      branch: "main",
    });
    const record = (await response.json()) as ServerUpdateRun;
    ...
    expect(composeUp[0]?.env?.FURLPAY_IMAGE_TAG).toBe(`sha-${targetCommit}`);
    expect(composeUp[1]?.env?.FURLPAY_IMAGE_TAG).toBe("v1.0.0");
  ...
  it("resets a fork checkout when recreate fails after the fast-forward", async () => {
    const fixture = await deployment("FURLPAY_IMAGE_TAG=local\n");
    await mkdir(path.join(fixture.deployDir, ".git"));
    const calls: string[][] = [];
    let upCalls = 0;
    const run: UpdaterCommandRunner = async (command, args) => {
  ...
  it("refuses to replace a symlinked deployment environment", async () => {
    ...
    await writeFile(target, "FURLPAY_IMAGE_TAG=v1.0.0\n");
    await rm(envFile);
    ...
    expect(await readFile(target, "utf8")).toBe("FURLPAY_IMAGE_TAG=v1.0.0\n");
  });

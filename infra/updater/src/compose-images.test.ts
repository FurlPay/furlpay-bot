...
const FIRST_PARTY_IMAGE = /ghcr\.io\/furlpay\/furlpay-bot\/([a-z0-9][a-z0-9._-]*)/g;
...
describe("the images compose file", () => {
  it("runs postgres, app roles, supervisor, and a published computer image", () => {
    ...
    for (const service of appServices) {
      expect(compose.services[service]?.image).toContain("ghcr.io/furlpay/furlpay-bot/app");
      expect(compose.services[service]?.image).toContain("FURLPAY_IMAGE_TAG");
    }
    expect(compose.services.computer?.image).toContain("ghcr.io/furlpay/furlpay-bot/computer");
    expect(compose.services.computer?.image).toContain("FURLPAY_COMPUTER_IMAGE_TAG");
  ...
  it("skips non-string Compose environment scalars when collecting image names", () => {
    ...
    expect(firstPartyImageNames("ghcr.io/furlpay/furlpay-bot/computer:edge")).toEqual(["computer"]);
  });

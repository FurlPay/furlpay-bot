...
export function resolveUpdaterConfig(env: NodeJS.ProcessEnv): UpdaterConfig {
  const deployDir = env.FURLPAY_DEPLOY_DIR?.trim() ?? "";
  if (deployDir === "" || !path.posix.isAbsolute(deployDir)) {
    throw new Error("Set FURLPAY_DEPLOY_DIR to the absolute path of the deployment directory.");
  }
  const image = env.FURLPAY_IMAGE?.trim() || OFFICIAL_SERVER_IMAGE;
  if (!isValidImageName(image))
    throw new Error(`FURLPAY_IMAGE is not a usable image name: ${image}`);
  const composeFile = env.FURLPAY_COMPOSE_FILE?.trim() || DEFAULT_COMPOSE_FILE;
  if (path.posix.isAbsolute(composeFile) || composeFile.split("/").includes("..")) {
    throw new Error("FURLPAY_COMPOSE_FILE must be a path inside the deployment directory.");
  }
  const port = Number(env.FURLPAY_UPDATER_PORT ?? DEFAULT_UPDATER_PORT);
  if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
    throw new Error("FURLPAY_UPDATER_PORT is not a port number.");
  }
  return {
    deployDir,
    ...
    host: env.FURLPAY_UPDATER_HOST?.trim() || "127.0.0.1",
    port,
  };
}

...
import { RECREATED_SERVICES } from "@furlpay-bot/core";
...
describe("the updater compose service", () => {
  ...
  it("is bind-mounted at the same path it has on the host", () => {
    const mount = (updater.volumes ?? []).find((volume) => volume.includes("FURLPAY_DEPLOY_DIR"));
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    const deployDir = "${FURLPAY_DEPLOY_DIR:-/srv/furlpay-bot}";
    ...
    expect(updater.environment?.FURLPAY_DEPLOY_DIR).toBe(deployDir);
  ...
  it("pins its own image tag separately from the application image", () => {
    expect(updater.image).toContain("FURLPAY_UPDATER_IMAGE_TAG");
    for (const service of RECREATED_SERVICES) {
      expect(compose.services[service]?.image).toContain("FURLPAY_IMAGE_TAG");
    }
  });

  it("uses the official registry namespace and digest-pins third-party runtime images", () => {
    expect(updater.image).toContain("ghcr.io/furlpay/furlpay-bot/updater");
    expect(compose.services.api?.image).toContain("ghcr.io/furlpay/furlpay-bot/app");
  ...
  it("injects the actual Compose project name into the updater container", () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.COMPOSE_PROJECT_NAME).toBe("${COMPOSE_PROJECT_NAME:-furlpay-bot-prod}");
  });

  it("does not load the application env_file into the root-equivalent process", () => {
    expect(updater.env_file).toBeUndefined();
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.FURLPAY_UPDATER_TOKEN).toBe("${FURLPAY_UPDATER_TOKEN:-}");
  });
  ...
  it("does not let the api container reach the Docker socket to update itself", () => {
    ...
    expect(compose.services.api?.environment?.FURLPAY_UPDATER_URL).toBe("http://updater:7092");
  });
});

import { DEFAULT_COMPOSE_PROJECT_NAME, isLocalImageTag } from "@furlpay-bot/core";
...
const base = {
  ...
  FURLPAY_UPDATER_TOKEN: "fake-review-updater-token-000000000000",
} as const;
...
describe("resolveUpdaterConfig", () => {
  it("derives the compose file, env file, and defaults from the deployment directory", () => {
    ...
    expect(config).toMatchObject({
      deployDir: "/srv/furlpay-bot",
      composeFile: `/srv/furlpay-bot/${DEFAULT_COMPOSE_FILE}`,
      envFile: "/srv/furlpay-bot/.env",
      projectName: DEFAULT_COMPOSE_PROJECT_NAME,
      token: base.FURLPAY_UPDATER_TOKEN,
  ...
  it("uses the Compose-injected project name so -p matches the running stack", () => {
    ...
    expect(
      resolveUpdaterConfig({ ...base, FURLPAY_COMPOSE_PROJECT_NAME: "manual-stack" }).projectName,
    ).toBe("manual-stack");
  });
  ...
  it("binds to loopback unless the deployment says otherwise, so a stray port is not a door", () => {
    ...
    expect(resolveUpdaterConfig({ ...base, FURLPAY_UPDATER_HOST: "0.0.0.0" }).host).toBe("0.0.0.0");
  });

  it("refuses a deployment directory that is missing or relative", () => {
    expect(() => resolveUpdaterConfig({ FURLPAY_UPDATER_TOKEN: "t" })).toThrow(/FURLPAY_DEPLOY_DIR/);
    expect(() => resolveUpdaterConfig({ ...base, FURLPAY_DEPLOY_DIR: "srv/furlpay-bot" })).toThrow(
      /FURLPAY_DEPLOY_DIR/,
    );
  });
  ...
  it("refuses a compose path that escapes the deployment directory", () => {
    for (const composeFile of ["/etc/compose.yml", "../../etc/compose.yml", "a/../../b.yml"]) {
      expect(() => resolveUpdaterConfig({ ...base, FURLPAY_COMPOSE_FILE: composeFile })).toThrow(
        /FURLPAY_COMPOSE_FILE/,
      );
    }
  });

  it("refuses an image name it would not be willing to hand to compose", () => {
    expect(() => resolveUpdaterConfig({ ...base, FURLPAY_IMAGE: "Bad Name" })).toThrow(
      /FURLPAY_IMAGE/,
    );
    expect(resolveUpdaterConfig({ ...base, FURLPAY_IMAGE: "ghcr.io/me/app" }).image).toBe(
      "ghcr.io/me/app",
    );
  });

  it("refuses a port that is not a port", () => {
    expect(() => resolveUpdaterConfig({ ...base, FURLPAY_UPDATER_PORT: "0" })).toThrow(/port/);
    expect(() => resolveUpdaterConfig({ ...base, FURLPAY_UPDATER_PORT: "seven" })).toThrow(/port/);
  });
});

describe("readEnvAssignment", () => {
  it("reads the last assignment, ignoring comments and blank lines", () => {
    const contents = ["# FURLPAY_IMAGE_TAG=commented", "", "A=1", "A=2"].join("\n");
    expect(readEnvAssignment(contents, "A")).toBe("2");
    expect(readEnvAssignment(contents, "FURLPAY_IMAGE_TAG")).toBeNull();
  });
  ...
  it("does not match a key that merely shares a prefix", () => {
    expect(readEnvAssignment("FURLPAY_IMAGE_TAG_PREVIOUS=v1", "FURLPAY_IMAGE_TAG")).toBeNull();
  });
});

describe("readTagState", () => {
  it("reads the pinned tag and the rollback tag", () => {
    const contents = "FURLPAY_IMAGE_TAG=v1.1.0\nFURLPAY_IMAGE_TAG_PREVIOUS=v1.0.0\n";
  ...
  it("ignores values in the file that are not usable tags", () => {
    const contents = "FURLPAY_IMAGE_TAG=-rm\nFURLPAY_IMAGE_TAG_PREVIOUS=$(id)\n";

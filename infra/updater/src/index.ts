...
export function createUpdaterApp(
  ...
  async function writeEnvAssignments(assignments: Record<string, string>) {
    ...
    const temporary = `${config.envFile}.furlpay-bot-update-${randomUUID()}`;
  ...
  async function apply(request: { repoUrl: string; branch: string; official: boolean }) {
    ...
    if (decision.strategy === "build") {
      if (!checkout.present) {
        throw new UpdateRefused(
          "Building a fork needs the deployment's git checkout, and FURLPAY_DEPLOY_DIR has no .git directory. Clone the fork to the deployment directory, or switch back to the official repository to use published images.",
        );
      }
...
function startUpdater() {
  ...
  return serve({ fetch: app.fetch, hostname: config.host, port: config.port }, () => {
    console.log(`furlpay-bot updater on http://${config.host}:${config.port} for ${config.deployDir}`);
  });
}

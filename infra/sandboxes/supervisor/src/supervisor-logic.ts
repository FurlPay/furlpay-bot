...
import { canReleaseScreenLease, canTakeScreenLease } from "@furlpay-bot/core";
import { z } from "zod";
import {
  type SandboxInput,
  screenPorts,
...
export function hasComputerIdentity(
  ...
  const labeledSpaceId = labels?.["furlpay-bot.spaceId"] ?? labels?.["furlpay-bot.workspaceId"];
  return labels?.["furlpay-bot.botId"] === botId && labeledSpaceId === spaceId;
}
...
export function stopExtraScreenCommand(index: number) {
  ...
  const profile = `/home/furlpay-bot/.browser-profiles/chromium-screen-${layout.displayNumber}`;
...
export function ensureScreenCommand(index: number) {
  ...
  return [
    ...
    `cp /etc/furlpay-bot/fluxbox/init ${fluxHome}/.fluxbox/init`,
    `cp /etc/furlpay-bot/fluxbox/apps ${fluxHome}/.fluxbox/apps 2>/dev/null || true`,
    `cp /etc/furlpay-bot/fluxbox/menu ${fluxHome}/.fluxbox/menu 2>/dev/null || true`,
    `HOME=${fluxHome} DISPLAY=${layout.display} fluxbox -rc ${fluxHome}/.fluxbox/init >${log}-fluxbox.log 2>&1 &`,
    `if [ -d /home/furlpay-bot/.browser-profiles/chromium ]; then cp -a /home/furlpay-bot/.browser-profiles/chromium/. ${profile}/; rm -f ${profile}/SingletonLock ${profile}/SingletonCookie ${profile}/SingletonSocket; fi`,
    `DISPLAY=${layout.display} HOME=/home/furlpay-bot furlpay-bot-browser --user-data-dir=${profile} >${log}-browser.log 2>&1 &`,
...
export function workspaceTarget(relative: string) {
  return relative ? path.posix.join("/home/furlpay-bot", relative) : "/home/furlpay-bot";
}
...
export function interactiveScreenCommand(
  ...
  const tokenFile =
    layout.displayNumber === 1
      ? "/tmp/furlpay-bot/control-token"
      : `/tmp/furlpay-bot/control-token-${layout.displayNumber}`;
  const stopProcesses =
    `pkill -f '^x11vnc .* -rfbport ${layout.controlVncPort}' || true; ` +
    `pkill -f '^/usr/bin/python3 .*websockify.*${layout.controlPort}' || true; ` +
    `rm -f ${tokenFile}`;
  ...
  return [
    ...
    `(x11vnc -display ${layout.display} -forever -shared -nopw -listen 127.0.0.1 -rfbport ${layout.controlVncPort} -xkb -ncache 0 >/tmp/furlpay-bot/x11vnc-control-${layout.displayNumber}.log 2>&1 &)`,
    `(websockify --heartbeat=30 --web=/usr/share/novnc 0.0.0.0:${layout.controlPort} 127.0.0.1:${layout.controlVncPort} >/tmp/furlpay-bot/novnc-control-${layout.displayNumber}.log 2>&1 &)`,

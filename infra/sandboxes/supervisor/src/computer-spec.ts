...
export const COMPUTER_IMAGE = process.env.FURLPAY_COMPUTER_IMAGE ?? "furlpay-bot/computer:local";
...
export function containerCreateOptions(input: ComputerCreateInput) {
  ...
  return {
    ...
    Env: [
      ...
      "PIP_USER=1",
      ...(input.controlToken ? [`FURLPAY_COMPUTER_CONTROL_TOKEN=${input.controlToken}`] : []),
    ],
    Labels: {
      "furlpay-bot.managed": "true",
      "furlpay-bot.botId": input.botId,
      "furlpay-bot.spaceId": input.spaceId,
    },
    ExposedPorts: ports.ExposedPorts,
    HostConfig: {
      Binds: [`${input.homePath}:/home/furlpay-bot`],
...
export function containerNameFor(botId: string) {
  return `furlpay-bot-bot-${sanitizeIdentifier(botId)}`;
}
...
export function computerNetworkNameFor(botId: string) {
  ...
  return `furlpay-bot-computer-${sanitizeIdentifier(botId).slice(0, 32)}-${hash}`;
}
...
export function computerNetworkNamesForCleanup(botId: string) {
  ...
  return [
    ...
    `furlpay-bot-computer-${safe}`,
    `furlpay-bot-computer-${safe.slice(0, 32)}-${digest.slice(0, 8)}`,
  ];
}

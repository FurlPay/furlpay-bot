...
describe("graphical computer spec", () => {
  it("creates a VNC desktop, not an alpine sleep fallback", () => {
    const options = containerCreateOptions({
      name: "furlpay-bot-bot-abc",
      image: COMPUTER_IMAGE,
      botId: "abc",
      spaceId: "ws",
      homePath: "/var/furlpay-bot/homes/abc",
      networkMode: "furlpayBot_default",
    });
    expect(options.Image).toBe("furlpay-bot/computer:local");
    ...
    expect(options.HostConfig.Binds).toEqual(["/var/furlpay-bot/homes/abc:/home/furlpay-bot"]);
    expect(options.Env).toContain(
      "PATH=/home/furlpay-bot/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
    );
    expect(options.Env).toContain("NPM_CONFIG_PREFIX=/home/furlpay-bot/.local");
    ...
    expect(options.HostConfig.NetworkMode).toBe("furlpayBot_default");
  });
  ...
  it("still publishes host ports when NetworkMode is a per-bot isolated network", () => {
    ...
    const options = containerCreateOptions({
      ...
      botId: "bot_isolation",
      spaceId: "ws",
      homePath: "/var/furlpay-bot/homes/bot_isolation",
      networkMode,
    });
    expect(networkMode).toMatch(/^furlpay-bot-computer-bot_isolation-[0-9a-f]{32}$/);
  ...
  it("lists prior network name variants for cleanup", () => {
    ...
    expect(names).toContain("furlpay-bot-computer-bot_1");
  ...
  it("ships a browser desktop, not a fullscreen terminal", () => {
    ...
    const browser = readFileSync(path.join(root, "furlpay-bot-browser"), "utf8");
    const desktop = readFileSync(path.join(root, "furlpay-bot-browser.desktop"), "utf8");
    expect(dockerfile).toMatch(/chromium/);
    expect(dockerfile).toMatch(/furlpay-bot-browser\.desktop/);
    expect(dockerfile).toMatch(/control.py/);
    expect(dockerfile).toMatch(/USER 1000:1000/);
    expect(start).toMatch(/furlpay-bot-computer-control/);
    expect(start).toMatch(/furlpay-bot-browser/);
    expect(start).toMatch(/SingletonLock/);
    expect(start).toMatch(/xdg-mime default furlpay-bot-browser\.desktop/);
    ...
    expect(start).toMatch(/failed to register furlpay-bot-browser/);
    expect(start).toMatch(/failed to set default web browser/);
    expect(start).toMatch(/xdg-settings set default-web-browser furlpay-bot-browser\.desktop/);
    expect(start).not.toMatch(/xdg-mime default furlpay-bot-browser\.desktop .*\|\| true/);
    ...
    expect(desktop).toMatch(/Exec=\/usr\/local\/bin\/furlpay-bot-browser %U/);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const root = path.resolve(import.meta.dirname, "../../computer");
      const temp = mkdtempSync(path.join(tmpdir(), "furlpay-bot-browser-wrapper-"));
      ...
      const run = (display: string, args: string[] = []) => {
        const result = spawnSync("bash", [path.join(root, "furlpay-bot-browser"), ...args], {
          env: {
            ...process.env,
            DISPLAY: display,
            HOME: home,
            PATH: `${bin}${path.delimiter}${process.env.PATH ?? ""}`,
            FURLPAY_TEST_ARGS: capture,
          },
          encoding: "utf8",
        });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const root = path.resolve(import.meta.dirname, "../../computer");
      const temp = mkdtempSync(path.join(tmpdir(), "furlpay-bot-browser-crash-"));
      const bin = path.join(temp, "bin");
      const home = path.join(temp, "home");
      const chromium = path.join(bin, "chromium");
      const capture = path.join(temp, "args");
      ...
      try {
        const result = spawnSync("bash", [path.join(root, "furlpay-bot-browser")], {
          env: {
            ...process.env,
            DISPLAY: ":1",
            HOME: home,
        ...
        const skipped = spawnSync("bash", [path.join(root, "furlpay-bot-browser")], {
          env: {
            ...process.env,
            DISPLAY: ":1",
            HOME: home,
  ...
  it("keeps container names stable so a bot can resume", () => {
    expect(containerNameFor("bot_1")).toBe("furlpay-bot-bot-bot_1");
  ...
  it("uses the container IP only for the internal screen network topology", () => {
    const networkMode = "furlpayBot_default";
    expect(
      resolveScreenPublishTarget({
    ...
    expect(
      resolveScreenPublishTarget({
        ...
        networkMode: "furlpay-bot-computer-bot-1",
        networks: { "furlpay-bot-computer-bot-1": { IPAddress: "172.20.0.4" } },
        hostPort: "49152",
        containerPort: "6080",
      }),
  ...
  it("does not publish computer control port 7070 on the host", () => {
    const options = containerCreateOptions({
      name: "furlpay-bot-bot-ctrl",
      image: COMPUTER_IMAGE,
      botId: "ctrl",
      spaceId: "ws",
      homePath: "/var/furlpay-bot/homes/ctrl",
    });
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          "assert allow(['env', 'DISPLAY=:1', 'furlpay-bot-browser'], ':1')",
          "assert allow(['env', 'DISPLAY=:2', 'furlpay-bot-browser', 'https://example.com'], ':2')",
          "assert allow(['env', 'DISPLAY=:1', 'xterm'], ':1')",
          "assert long_lived(['env', 'DISPLAY=:1', 'furlpay-bot-browser'])",
          "assert long_lived(['env', 'DISPLAY=:1', 'furlpay-bot-browser', 'https://example.com'])",

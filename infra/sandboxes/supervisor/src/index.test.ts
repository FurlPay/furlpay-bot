...
import { resolveSupervisorToken } from "@furlpay-bot/core";
  ...
  it("rejects a provision request whose identity headers do not match its body", async () => {
    const response = await supervisorApp.request("/computers", {
      ...
      headers: {
        ...
        "x-furlpay-bot-bot-id": "other-bot",
        "x-furlpay-bot-space-id": "workspace",
      },
      body: JSON.stringify({
        botId: "bot",
        spaceId: "workspace",
...
describe("sandbox supervisor input containment", () => {
  ...
  it("accepts the legacy workspace label without weakening container identity", () => {
    expect(
      hasComputerIdentity({ "furlpay-bot.botId": "bot", "furlpay-bot.workspaceId": "space" }, "bot", "space"),
    ).toBe(true);
    expect(
      hasComputerIdentity(
        { "furlpay-bot.botId": "bot", "furlpay-bot.workspaceId": "other-space" },
        "bot",
        "space",
      ),
    ).toBe(false);
    expect(
      hasComputerIdentity(
        {
          "furlpay-bot.botId": "bot",
          "furlpay-bot.spaceId": "space",
          "furlpay-bot.workspaceId": "other-space",
        },
        "bot",
        "space",
      ),
  ...
  it("routes Docker browser aliases through the safe wrapper on every display", () => {
    for (const application of DOCKER_BROWSER_ALIASES) {
      expect(
        ...
        argv: ["env", "DISPLAY=:2", "furlpay-bot-browser", "https://example.com"],
      });
    }

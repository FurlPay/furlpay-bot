#!/usr/bin/env python3
"""Token-auth desktop control for the FurlPay Bot supervisor."""
...
TOKEN = os.environ.get("FURLPAY_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libfurlpay-bot-xcapture.so")
        library.furlpay-bot_xcapture_open.argtypes = [ctypes.c_char_p]
        library.furlpay-bot_xcapture_open.restype = ctypes.c_void_p
        library.furlpay-bot_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.furlpay-bot_xcapture_png.restype = ctypes.c_int
        library.furlpay-bot_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.furlpay-bot_xcapture_damage.restype = ctypes.c_int
        library.furlpay-bot_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.furlpay-bot_xinput_argv.restype = ctypes.c_int
        context = library.furlpay-bot_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.furlpay-bot_xcapture_png(
        ...
        changed = self.library.furlpay-bot_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.furlpay-bot_xinput_argv(self.context, len(argv), encoded)

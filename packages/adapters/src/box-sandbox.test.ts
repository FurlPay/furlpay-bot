  ...
  it("creates no-env boxes and implements execution and file operations", async () => {
    ...
    expect(fixture.create).toHaveBeenCalledWith(
      {
        createBoxRequest: {
          ttlSeconds: 7200,
          noEnv: true,
          env: { FURLPAY_BOT_ID: "bot-a", FURLPAY_SANDBOX: "computer" },
        },
      },
    ...
    expect(
      fixture.command.mock.calls.some(([request]) =>
        request.commandRequest.command.includes("furlpay-bot-home/.browser-profiles"),
      ),
    ).toBe(true);
    ...
    expect(executeRequest?.commandRequest.cwd).toBe("furlpay-bot-home/notes");
...
function boxFixture(options: { state?: string } = {}) {
  ...
  const fixture = {
    ...
    command: vi.fn(async (request: { commandRequest: { command: string; cwd?: string } }) => {
      ...
      if (command.includes("find ") && command.includes("furlpay-bot-home/bin")) {
        const target = "/home/user/furlpay-bot-home/bin/tool";
        return finished({
      ...
      if (
        ...
        command.includes("furlpay-bot-home") &&
        command.includes("-type f")
      ) {
        const stdout = [...files.entries()]
          .filter(([filePath]) => filePath.startsWith("/home/user/furlpay-bot-home/"))
    ...
    readFile: vi.fn(async ({ path }: { path: string }) => {
      const content = path.startsWith("/tmp/furlpay-bot-observe-")

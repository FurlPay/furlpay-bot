...
import { OPENAI_COMPATIBLE_PROVIDER_ID } from "@furlpay-bot/contracts";
...
export function openAiCompatAllowPublicHosts(): boolean {
  return process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC === "1";
}
...
export function assertAllowedOpenAiCompatibleRequestUrl(
  ...
  if (!allowPublic) {
    throw new Error(
      "Public model endpoints are blocked. Set FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC=1 to allow them.",
    );
  }
  return url;
}

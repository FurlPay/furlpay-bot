...
import { ACTIVE_RUN_STATUSES } from "@furlpay-bot/core";
...
const BACKGROUND_WORK_MARKER_PREFIX = "/tmp/furlpay-bot-background-";
const BACKGROUND_WORK_IDLE_SENTINEL = "furlpay-bot-background-idle";
...
async function hasActiveBackgroundWork(
  ...
  try {
    for await (const event of sandbox.execute(
      computer,
      {
        argv: ["bash", "-c", BACKGROUND_WORK_PROBE, "furlpay-bot-background-probe", computerId],
        timeoutMs: 10_000,
      },
      context,
    )) {

import type { AgentModelOAuthCredential, AgentRuntime } from "@furlpay-bot/adapter-kit";
import type { ActionApprovalRule } from "@furlpay-bot/core";
import { type AutoReviewJudgeDecision, redactSecrets } from "@furlpay-bot/core";
...
function localModelIds(env: NodeJS.ProcessEnv): string[] {
  return (env.FURLPAY_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
export function deploymentAutoReviewDefault(env: NodeJS.ProcessEnv = process.env): boolean {
  return envFlag(env, "FURLPAY_AUTO_REVIEW");
}

export function autoReviewTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.FURLPAY_AUTO_REVIEW_TIMEOUT_MS?.trim();
...
export function resolveAutoReviewChecker(
  ...
  const overrideProvider = env.FURLPAY_AUTO_REVIEW_PROVIDER?.trim();
  const overrideModel = env.FURLPAY_AUTO_REVIEW_MODEL?.trim();

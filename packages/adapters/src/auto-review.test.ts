...
describe("resolveAutoReviewChecker", () => {
  it("prefers explicit env overrides", () => {
    expect(
      resolveAutoReviewChecker({
        FURLPAY_AUTO_REVIEW_PROVIDER: "openrouter",
        FURLPAY_AUTO_REVIEW_MODEL: "cheap/fast",
        FURLPAY_LOCAL_MODELS: "local-a",
  ...
  it("prefers local models when configured", () => {
    expect(
      resolveAutoReviewChecker({
        FURLPAY_LOCAL_MODELS: " llama-local , other ",
...
describe("isAutoReviewCheckerConfigured", () => {
  it("requires a runnable checker", () => {
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { FURLPAY_LOCAL_MODELS: "local-1" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
...
describe("deploymentAutoReviewDefault", () => {
  it("defaults off", () => {
    ...
    expect(deploymentAutoReviewDefault({ FURLPAY_AUTO_REVIEW: "1" })).toBe(true);
    expect(deploymentAutoReviewDefault({ FURLPAY_AUTO_REVIEW: "true" })).toBe(true);
  });
});
...
describe("autoReviewTimeoutMs", () => {
  it("defaults to 1500 and clamps bad values", () => {
    ...
    expect(autoReviewTimeoutMs({ FURLPAY_AUTO_REVIEW_TIMEOUT_MS: "2000" })).toBe(2_000);
    expect(autoReviewTimeoutMs({ FURLPAY_AUTO_REVIEW_TIMEOUT_MS: "nope" })).toBe(1_500);
  });
});

...
import { isLocalMcpHost } from "@furlpay-bot/contracts";
import type { McpServer, PrismaClient } from "@furlpay-bot/db";
...
export class McpConnector implements ConnectorProvider {
  ...
  private async connectSession(server: McpServer, context: AdapterContext): Promise<McpSession> {
    const session = new McpSession({ name: `furlpay-bot-${server.slug}` });
    try {

...
const E2B_WORKSPACE = "/home/user/furlpay-bot-home";
...
export function e2bCreateOptions(botId: string, apiKey: string) {
  return {
    ...
    metadata: { botId, "furlpay-bot": "computer" },
...
export class E2BSandboxProvider implements SandboxProvider {
  ...
  private async startControlStream(
    ...
    if (layout.isPrimary) {
      const passwordFile = "/tmp/furlpay-bot-control.vncpass";
      const tokenFile = "/tmp/furlpay-bot-control.token";
      ...
      const command = [
        ...
        `x11vnc -bg -display ${shellQuote(desktop.display)} -forever -wait 50 -shared -rfbport ${vncPort} -rfbauth ${passwordFile} 2>/tmp/furlpay-bot-control-x11vnc.log`,
        // Require the new x11vnc itself — proxy listen alone can pass with a leftover server.
        `for i in $(seq 1 50); do netstat -tuln | grep -q ':${vncPort} ' && break; sleep 0.1; done`,
        `if ! netstat -tuln | grep -q ':${vncPort} '; then exit 1; fi`,
        "cd /opt/noVNC/utils",
        `(nohup ./novnc_proxy --vnc localhost:${vncPort} --listen ${proxyPort} --web /opt/noVNC >/tmp/furlpay-bot-control-novnc.log 2>&1 &)`,
...
function controlStreamStopCommand(controlToken?: string) {
  ...
  const stop = [
    ...
    "rm -f /tmp/furlpay-bot-control.vncpass",
    "rm -f /tmp/furlpay-bot-control.token",
  ].join("; ");
  if (!controlToken) return stop;
  return `[ -f /tmp/furlpay-bot-control.token ] && [ "$(cat /tmp/furlpay-bot-control.token)" != ${shellQuote(controlToken)} ] || { ${stop}; }`;
}

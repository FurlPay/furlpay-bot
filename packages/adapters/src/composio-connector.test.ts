...
import type { AdapterContext, ConnectorEvent, ConnectorTool } from "@furlpay-bot/adapter-kit";
...
describe("composio tool mapping", () => {
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__furlpayBot_catalog__", toolkits: undefined, sessionPreset: undefined },
  ...
  it("only fetches live Composio slugs when a FurlPay Bot row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but FurlPay Bot is still pending", () => {
    expect(
      planLiveConnectionSync(
        [

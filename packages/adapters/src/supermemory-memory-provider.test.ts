import type { AdapterContext } from "@furlpay-bot/adapter-kit";
...
describe("SupermemoryMemoryProvider", () => {
  it("keeps Supermemory namespaces inside the adapter and omits empty history", async () => {
    ...
    ).toEqual(["furlpay-bot:workspace:workspace-1", "furlpay-bot:bot-1"]);
  });
  ...
  it("adds the current history generation only after compaction", async () => {
    ...
    expect(
      fetchMock.mock.calls.map((call) => JSON.parse(String(call[1]?.body)).containerTag),
    ).toEqual(["furlpay-bot:bot-1", "furlpay-bot:bot-1:history:3"]);
  });
  ...
  it("purges only requested history generations", async () => {
    ...
    expect(fetchMock.mock.calls.map((call) => call[0])).toEqual([
      "http://localhost:6767/v3/container-tags/furlpay-bot%3Abot-1%3Ahistory%3A2",
      "http://localhost:6767/v3/container-tags/furlpay-bot%3Abot-1%3Ahistory%3A3",
    ]);
  });
});

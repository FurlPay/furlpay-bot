...
describe("desktop sandbox path rules", () => {
  it("compares Windows roots case-insensitively without accepting siblings or other drives", () => {
    const roots = ["C:\\Users\\Owner\\FurlPay Bot\\bot"];

    expect(
      isAllowedDesktopPath("c:\\users\\owner\\furlpay-bot\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("C:\\Users\\Owner\\FurlPay Bot\\bot-other\\notes.txt", roots, path.win32),
    ).toBe(false);
  ...
  it("handles explicitly allowed UNC roots without accepting sibling shares", () => {
    const roots = ["\\\\server\\share\\furlpay-bot\\bot"];

    expect(
      isAllowedDesktopPath("\\\\SERVER\\share\\furlpay-bot\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("\\\\server\\share-other\\furlpay-bot\\bot\\notes.txt", roots, path.win32),
    ).toBe(false);
  });

...
describe("extra display ports", () => {
  ...
  it("uses a locked sandbox registry for cross-process screen assignment", () => {
    ...
    expect(release).toContain("FURLPAY_SCREEN_RELEASE=stale");
    expect(release.indexOf("pkill -f")).toBeLessThan(release.indexOf('rm -f "$slot"'));
    expect(parseAllocatedExtraDisplay("FURLPAY_SCREEN_INDEX=3\n")).toBe(3);
    expect(parseReleasedExtraDisplay("FURLPAY_SCREEN_RELEASE=3\n")).toBe(3);
    expect(parseReleasedExtraDisplay("FURLPAY_SCREEN_RELEASE=stale\n")).toBeUndefined();
  });

  it("requires an authenticated password for view-only VNC", () => {
    expect(parseExtraDisplayViewPassword("FURLPAY_SCREEN_PASSWORD=sandbox_secret-1\n")).toBe(
      "sandbox_secret-1",
    );

import type { ProcessEvent } from "@furlpay-bot/adapter-kit";
...
describe("Docker sandbox", () => {
  ...
  it("sends the bounded timeout to the supervisor and preserves its honest result", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[1]?.headers).not.toHaveProperty("x-furlpay-bot-screen-id");
  });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          ...
          "x-furlpay-bot-bot-id": "home-bot",
          "x-furlpay-bot-screen-lease-id": "run-1:1",

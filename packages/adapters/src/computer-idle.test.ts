...
import type { AgentHomeStore, JobPublisher, SandboxProvider } from "@furlpay-bot/adapter-kit";
  ...
  it("probes background work with the database computer id, not providerRef", async () => {
    ...
    expect(harness.sandbox.execute).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        argv: ["bash", "-c", BACKGROUND_WORK_PROBE, "furlpay-bot-background-probe", harness.computer.id],
      }),
      expect.anything(),
    );
...
describe("background work launch and probe", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      markers.add(`/tmp/furlpay-bot-background-${databaseId}-${launchId}`);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const completedMarker = `/tmp/furlpay-bot-background-${markerId}-completed`;
      const activeMarker = `/tmp/furlpay-bot-background-${markerId}-active`;
      markers.add(completedMarker);
      markers.add(activeMarker);
      const completed = spawn(
        "bash",
        ["-c", BACKGROUND_WORK_LAUNCH, "furlpay-bot-background-launch", markerId, "completed", "true"],
        { stdio: "ignore" },
      );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const marker = `/tmp/furlpay-bot-background-${markerId}-collision`;
      const commandRan = `/tmp/furlpay-bot-background-command-ran-${markerId}`;
      markers.add(marker);
  ...
  const sandbox = {
    execute: vi.fn(async function* () {
      ...
      if (code === 1 && !options.backgroundWorkProbeFailed) {
        yield { type: "stdout", data: "furlpay-bot-background-idle\n" } as const;
      }
...
function probeBackgroundWork(markerId: string): Promise<number> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      "bash",
      ["-c", BACKGROUND_WORK_PROBE, "furlpay-bot-background-probe", markerId],

    ...
    prepareArguments: (args: unknown) => {
      ...
      if (tool.name === "destination.write") {
        return {
          ...
          title: String(raw.title ?? "FurlPay Bot result"),
      ...
      if (tool.name === "shell") {
        return {
          ...
          cwd: raw.cwd ? String(raw.cwd) : "/home/furlpay-bot",
        };
      }
...
async function executeSubagent(host: ToolHost, executionId: string, args: Record<string, unknown>) {
  ...
  const nested = new Agent({
    ...
    initialState: {
      systemPrompt: [
        `You are a FurlPay Bot subagent named "${name}".`,

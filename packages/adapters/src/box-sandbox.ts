...
import { boundedSandboxCommandTimeoutMs } from "@furlpay-bot/core";
...
const BOX_WORKSPACE = "/home/user/furlpay-bot-home";
...
export class BoxSandboxProvider implements SandboxProvider {
    ...
    const created = await this.client.create(
      {
        createBoxRequest: {
          ...
          noEnv: true,
          env: {
            FURLPAY_BOT_ID: request.botId,
            FURLPAY_SANDBOX: "computer",
          },
        },
      },
      { signal: context.signal },
  ...
  async observe(computer: ComputerRef, context: AdapterContext): Promise<ComputerObservation> {
    const id = this.id(computer);
    this.screens.claim(id, context);
    const imagePath = `/tmp/furlpay-bot-observe-${randomUUID()}.png`;
    try {
      const result = await this.runCommand(
        id,
  ...
  private async runCommand(
    ...
    const marker = `/tmp/furlpay-bot-command-${randomUUID()}.completed-124`;
  ...
  private async terminateCommand(id: string, marker: string): Promise<void> {
    const pattern = marker.replace("furlpay-bot", "[r]akazo");
    await this.rawCommand(
      id,
...
function boxCwd(cwd: string | undefined): string {
  if (
    ...
    cwd === "." ||
    cwd === "/" ||
    cwd === "/home/furlpay-bot" ||
    cwd === "/home/user" ||
    cwd === BOX_WORKSPACE
  ) {
    return "furlpay-bot-home";
  }
  const relative = cwd.startsWith(`${BOX_WORKSPACE}/`)
    ? cwd.slice(BOX_WORKSPACE.length + 1)
    : cwd.startsWith("/home/furlpay-bot/")
      ? cwd.slice("/home/furlpay-bot/".length)
      : cwd;
  return path.posix.join("furlpay-bot-home", normalizeWorkspacePath(relative));
}
...
function launchBoxBrowserCommand(): string {
  return [
    ...
    '    nohup env DISPLAY=:0 "$app" --no-first-run --no-default-browser-check --start-maximized >/tmp/furlpay-bot-browser.log 2>&1 &',
    "    sleep 1",
...
function boxActionCommand(action: Exclude<ComputerAction, { kind: "wait" }>): string {
  ...
  if (action.kind === "open") {
    ...
    return `nohup env DISPLAY=:0 xdg-open ${shellQuote(target)} >/tmp/furlpay-bot-open.log 2>&1 &`;
  }
  const applications =
  ...
  return [
    ...
    '  if command -v "$app" >/dev/null 2>&1; then',
    `    nohup env DISPLAY=:0 "$app"${action.uri ? ` ${shellQuote(action.uri)}` : ""} >/tmp/furlpay-bot-app.log 2>&1 &`,
    "    exit 0",
    "  fi",
    "done",
    "exit 1",

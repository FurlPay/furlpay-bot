...
export class DaytonaSandboxProvider implements SandboxProvider {
  ...
  async provision(
    ...
    const sandbox = await this.client.create(
      {
        labels: { botId: request.botId, "furlpay-bot": "computer" },
  ...
  private async workspaceRoot(sandbox: Sandbox): Promise<string> {
    ...
    const root = path.posix.join(home, "furlpay-bot-home");
...
async function launchDaytonaApplication(
  ...
  const command = [
    ...
    `    nohup "$app"${uri ? ` ${shellQuote(uri)}` : ""} >/tmp/furlpay-bot-app.log 2>&1 &`,
    "    exit 0",
    "  fi",
    "done",
    "exit 1",

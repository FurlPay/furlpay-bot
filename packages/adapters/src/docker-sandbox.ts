...
export class DockerSandboxProvider implements SandboxProvider {
  ...
  // No x-furlpay-bot-screen-id here: the supervisor keys a screen off
  // x-furlpay-bot-bot-id alone (the ComputerRef's homeKey — shared across every
  ...
  private headers(context: AdapterContext, botId?: string) {
    return {
      ...
      "x-furlpay-bot-space-id": context.spaceId,
      ...(botId ? { "x-furlpay-bot-bot-id": botId } : {}),
      ...(context.screenLeaseId ? { "x-furlpay-bot-screen-lease-id": context.screenLeaseId } : {}),
    };
  }
...
function dockerCwd(cwd: string | undefined) {
  if (!cwd || cwd === "." || cwd === "/" || cwd === "/home/furlpay-bot") return "/home/furlpay-bot";
  const relative = cwd.startsWith("/home/furlpay-bot/")
    ? cwd.slice("/home/furlpay-bot/".length)
    : normalizeWorkspacePath(cwd);
  return path.posix.join("/home/furlpay-bot", relative);
}

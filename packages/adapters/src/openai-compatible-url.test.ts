...
const savedAllowPublic = process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC;

afterEach(() => {
  if (savedAllowPublic === undefined) delete process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC;
  else process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC = savedAllowPublic;
});
...
describe("openai-compatible URL policy", () => {
  ...
  it("allows public bigmodel host when ALLOW_PUBLIC=1 without rewriting /v4", () => {
    process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC = "1";
  ...
  it("rejects public hosts unless explicitly allowed", () => {
    delete process.env.FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC;

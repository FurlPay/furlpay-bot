...
import type { PortableFile, SandboxProvider } from "@furlpay-bot/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "furlpay-bot-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `furlpay-bot-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [

...
import type { ModelOAuthSignInMode, ThinkingLevel } from "@furlpay-bot/contracts";
...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from FurlPay Bot.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control. FurlPay Bot does not pay for model usage.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the FurlPay Bot UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. FurlPay Bot does not pay for model usage.`;
  }
  return `Uses your ${name} key. FurlPay Bot does not pay for model usage.`;
}

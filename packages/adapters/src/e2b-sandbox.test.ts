...
describe("E2B computer backend", () => {
  ...
  it("opens http(s) URLs through the named browser launcher", async () => {
    const command = vi.fn(async (value: string) => {
      if (value.includes("FURLPAY_SCREEN_INDEX=")) {
        return { stdout: "FURLPAY_SCREEN_INDEX=0\n", stderr: "", exitCode: 0 };
      }
    ...
    expect(open).toHaveBeenCalledWith("/home/user/furlpay-bot-home/notes/readme.md");
  });
  ...
  it("controls the desktop and exposes a portable workspace", async () => {
    ...
    const command = vi.fn(async (value: string, _options?: Record<string, unknown>) => {
      ...
      if (value.includes("FURLPAY_SCREEN_RELEASE=")) {
        return {
          stdout: "FURLPAY_SCREEN_RELEASE=0\n",
          stderr: "",
          exitCode: 0,
        };
      }
  ...
  it("gives Team bots distinct E2B screens and shared files", async () => {
    ...
    const command = vi.fn(async (value: string) => {
      ...
      if (screenKey && value.includes("FURLPAY_SCREEN_INDEX=")) {
        ...
        return {
          stdout: `FURLPAY_SCREEN_INDEX=${index}\n`,
          stderr: "",
          exitCode: 0,
        };
      }
      if (screenKey && value.includes("FURLPAY_SCREEN_RELEASE=")) {
        const index = screenSlots.get(screenKey);
        if (index === undefined) {
          return {
            stdout: "FURLPAY_SCREEN_RELEASE=missing\n",
            stderr: "",
            exitCode: 0,
          };
        }
        ...
        return {
          stdout: `FURLPAY_SCREEN_RELEASE=${index}\n`,
          stderr: "",
          exitCode: 0,
        };
      }
      if (value.includes("command -v Xvfb")) return { stdout: "", stderr: "", exitCode: 0 };
      if (value.includes("FURLPAY_SCREEN_PASSWORD=")) {
        return {
          stdout: "FURLPAY_SCREEN_PASSWORD=test-view-password\n",
          stderr: "",
          exitCode: 0,
        };
      }
...
describe("sandbox-gone detection", () => {
  ...
  const alive = [
    ...
    new Error("Path /home/user/furlpay-bot-home/notes.md not found"),

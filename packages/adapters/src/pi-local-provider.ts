...
export function localBaseUrl(): string {
  const value = process.env.FURLPAY_LOCAL_MODELS_URL?.trim() || DEFAULT_BASE_URL;
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    throw new Error("FURLPAY_LOCAL_MODELS_URL must be an absolute HTTP(S) URL");
  }
...
function localModelIds(): string[] {
  return (process.env.FURLPAY_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
function localModel(id: string): Model<"openai-completions"> {
  return {
    ...
    contextWindow: tokenLimit("FURLPAY_LOCAL_CONTEXT_WINDOW", DEFAULT_CONTEXT_WINDOW),
    maxTokens: tokenLimit("FURLPAY_LOCAL_MAX_TOKENS", DEFAULT_MAX_TOKENS),
  };
}

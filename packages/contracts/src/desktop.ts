...
export interface FurlPayBotDesktopUpdate {
...
export interface FurlPayBotDesktopOAuthCallback {
  code: string;
  state?: string;
}

export interface FurlPayBotDesktop {
  platform: string;
  window: {
    close: () => Promise<void>;
  ...
  update: FurlPayBotDesktopUpdate;
  oauth: {
    /**
     * Authorization codes captured from a sign-in popup's loopback redirect.
     * Returns an unsubscribe function.
     */
    onCallback: (listener: (callback: FurlPayBotDesktopOAuthCallback) => void) => () => void;
  };
}

/** How the desktop app was pointed at a FurlPay Bot server during first-run setup. */
 ...
 * narrower `furlpayBotDesktop` bridge so a connected server can never re-point the app.
 */
export interface FurlPayBotSetup {

...
export const GroupAvatar = memo(function GroupAvatar({
  ...
  if (!firstMember) {
    return (
      <div
        className={cn(
          "furlpay-bot-group-avatar relative flex items-center justify-center rounded-full border border-[var(--fp-border)] bg-[var(--fp-surface-2)] text-[var(--fp-muted)]",
          className,
        )}
  ...
  if (members.length === 1) {
    return (
      <BotAvatar
        ...
        className={cn("furlpay-bot-group-avatar", className)}
      />
    );
  }
  ...
  return (
    <div
      className={cn("furlpay-bot-group-avatar relative rounded-full select-none", className)}
      style={{ width: size, height: size, flex: "none" }}
    >
      {visibleMembers.map((member, index) => (
        <div
          ...
          style={{
            ...positions[index],
            zIndex: index + 1,
            boxShadow: "0 0 0 1.5px var(--fp-elevated)",
          }}
        >
          <BotAvatar
            color={member.color}
      ...
      {members.length > 3 ? (
        <div
          className="absolute right-0 bottom-0 z-[3] flex items-center justify-center rounded-full bg-[var(--fp-elevated)] text-[10px] font-semibold text-[var(--fp-ink)]"
          style={{
            width: miniSize,
            height: miniSize,
            boxShadow: "0 0 0 1.5px var(--fp-elevated)",

import { ACTIVE_RUN_STATUSES, avatarIdentitySeed, organicAvatarPath } from "@furlpay-bot/core";
...
export const BotAvatar = memo(function BotAvatar({
  ...
  const idleEyeAnimation = {
    "--furlpay-bot-eye-animation-name": `furlpay-bot-eyes-idle-${eyeVariant}`,
    "--furlpay-bot-eye-animation-duration": `${idleDuration}s`,
    "--furlpay-bot-eye-animation-easing": "cubic-bezier(0.4, 0, 0.2, 1)",
    "--furlpay-bot-eye-animation-delay": `${idleDelay}s`,
  } as CSSProperties;
  const workingEyeAnimation = {
    "--furlpay-bot-eye-animation-name": "furlpay-bot-eyes-working",
    "--furlpay-bot-eye-animation-duration": "1.4s",
    "--furlpay-bot-eye-animation-easing": "ease-in-out",
    "--furlpay-bot-eye-animation-delay": "0s",
  } as CSSProperties;

  return (
    <div
      className={cn(
        "furlpay-bot-bot-avatar group relative flex items-center justify-center rounded-full select-none",
        className,
      )}
    ...
    >
      <svg
        className="furlpay-bot-bot-avatar-ring absolute pointer-events-none"
        style={{
          inset: -4,
          width: size + 8,
          height: size + 8,
      ...
      <div
        className="furlpay-bot-bot-avatar-visor relative flex items-center justify-center overflow-hidden transition-transform duration-200 group-hover:scale-[1.04]"
        style={{
          width: visorW,
          height: visorH,
      ...
      >
        ...
        {(["idle", "working"] as const).map((mode) => (
          <div
            key={mode}
            className={`furlpay-bot-bot-avatar-eyes furlpay-bot-bot-avatar-eyes-${mode} absolute inset-0 z-10 flex items-center justify-center`}
            style={{
              gap: eyeGap,
...
function OrganicAvatar({
  ...
  return (
    <svg
      ...
      aria-hidden="true"
      className={cn("furlpay-bot-organic-avatar overflow-visible select-none", className)}
      data-working={isWorking}
    ...
    >
      {(["idle", "working"] as const).map((mode) => (
        <path
          key={mode}
          className={`furlpay-bot-organic-avatar-body furlpay-bot-organic-avatar-body-${mode}`}
          d={shapeA}
          fill={color}
          style={
            {
              "--furlpay-bot-organic-path": `path("${shapeA}")`,
              filter:
                mode === "working"
      ...
      <g transform={`rotate(${(seed % 9) - 4})`}>
        {(["idle", "working"] as const).map((mode) => (
          <g
            key={mode}
            className={`furlpay-bot-organic-avatar-eyes furlpay-bot-organic-avatar-eyes-${mode}`}
            fill="#101014"
          >
...
export function Wordmark({ className }: { className?: string }) {
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <div className="flex h-11 w-11 items-center justify-center gap-1.5 rounded-full bg-[var(--fp-surface)]">
        <span className="h-4 w-[7px] rounded-full bg-[var(--fp-cream)]" />
        <span className="h-4 w-[7px] rounded-full bg-[var(--fp-cream)]" />
      </div>
      <span className="font-[Aeonik,ui-sans-serif] text-[28px] tracking-tight text-[var(--fp-ink-strong)]">
        FurlPay Bot
      </span>
    </div>
  );

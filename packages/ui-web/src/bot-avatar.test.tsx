...
import { avatarIdentitySeed } from "@furlpay-bot/core";
...
describe("BotAvatar", () => {
  ...
  it.each(["running", "queued", "leased", "waiting_input", "waiting_takeover"])(
    ...
    (status) => {
      ...
      expect(html).toContain("furlpay-bot-bot-avatar-ring");
    },
  );
  ...
  it("generates an organic avatar from the bot color", () => {
    ...
    expect(html).toContain("furlpay-bot-organic-avatar");
    expect(html).toContain('data-working="true"');
    expect(html).toMatch(/data-shape-family="\d"/);
    expect(html).toMatch(/data-eye-pattern="[0-3]"/);
    expect(html).toContain("<animate");
    expect(html).not.toContain("furlpay-bot-bot-avatar-visor");
  });
  ...
  it("keeps the organic morph timeline stable across status updates", () => {
    ...
    expect(idle).toContain("furlpay-bot-organic-avatar-eyes-idle");
    expect(idle).toContain("furlpay-bot-organic-avatar-eyes-working");
    expect(idle).toContain("furlpay-bot-organic-avatar-body-idle");
    expect(idle).toContain("furlpay-bot-organic-avatar-body-working");

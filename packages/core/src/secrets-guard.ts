...
const RUNTIME_SECRETS_ERROR =
  "Set BETTER_AUTH_SECRET and ENCRYPTION_KEY to long random strings before starting FurlPay Bot outside local development or tests.";
...
export function isDevSecretAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.FURLPAY_ALLOW_DEV_SECRETS === "1") return true;
...
export function resolveUpdaterToken(env: NodeJS.ProcessEnv = process.env): string {
  return resolveDedicatedSecret(env, {
    name: "FURLPAY_UPDATER_TOKEN",
    conflicts: ["BETTER_AUTH_SECRET", "SANDBOX_SUPERVISOR_TOKEN", "SCREEN_PROXY_SECRET"],
    missingMessage: "Set FURLPAY_UPDATER_TOKEN to a dedicated random updater credential.",
    conflictMessage:
      "FURLPAY_UPDATER_TOKEN must differ from BETTER_AUTH_SECRET, SANDBOX_SUPERVISOR_TOKEN, and SCREEN_PROXY_SECRET.",
  });
}
 ...
 * through `@furlpay-bot/core`, and importing `node:crypto` here fails the production Vite build with

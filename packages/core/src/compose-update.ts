 ...
 */
// MUST stay lowercase. This is interpolated straight into the GHCR image
// references below, and a Docker image reference may not contain uppercase, so
// an uppercase owner here is rejected by the registry at pull time rather than
// at build time — it fails on the operator's machine, not in our CI.
// The GitHub org is spelled "FurlPay"; only its image path is lowercased.
export const PUBLISHED_IMAGE_REPO = "furlpay/furlpay-bot";
...
export const IMAGE_TAG_ENV = "FURLPAY_IMAGE_TAG";
export const PREVIOUS_IMAGE_TAG_ENV = "FURLPAY_IMAGE_TAG_PREVIOUS";
 ...
 */
export const DEFAULT_COMPOSE_PROJECT_NAME = "furlpay-bot-prod";
export const COMPOSE_PROJECT_NAME_ENV = "COMPOSE_PROJECT_NAME";
export const COMPOSE_PROJECT_NAME_OVERRIDE_ENV = "FURLPAY_COMPOSE_PROJECT_NAME";
 ...
 * absent because neither uses the FurlPay Bot image.
 */
...
/**
 * `updaterUrlConfigured` means the API was given `FURLPAY_UPDATER_URL` (Compose prod always sets
...
export const SOURCE_MANUAL_UPGRADE_COMMANDS = [
  "git pull",
  "pnpm --filter @furlpay-bot/db migrate",

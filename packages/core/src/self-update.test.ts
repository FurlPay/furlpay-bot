...
describe("normalizeRepoUrl", () => {
  it("accepts the official repository and https forks", () => {
    expect(normalizeRepoUrl(OFFICIAL_REPO_URL)).toEqual({
      url: "https://github.com/FurlPay/furlpay-bot",
    });
    expect(normalizeRepoUrl("  https://github.com/me/furlpay-bot.git/  ")).toEqual({
      url: "https://github.com/me/furlpay-bot.git",
    });
    expect(normalizeRepoUrl("https://git.example.com:8443/team/furlpay-bot.git")).toEqual({
      url: "https://git.example.com:8443/team/furlpay-bot.git",
    });
  });

  it("accepts ssh remotes in both spellings", () => {
    expect(normalizeRepoUrl("git@github.com:me/furlpay-bot.git")).toEqual({
      url: "git@github.com:me/furlpay-bot.git",
    });
    expect(normalizeRepoUrl("ssh://git@github.com/me/furlpay-bot.git")).toEqual({
      url: "ssh://git@github.com/me/furlpay-bot.git",
    });
  });
  ...
  it("refuses transports and shapes that are not a git remote", () => {
    for (const input of [
      "",
      "   ",
      "http://github.com/me/furlpay-bot",
      "git://github.com/me/furlpay-bot",
      "file:///etc/passwd",
      "/srv/furlpay-bot",
      "https://github.com",
      "https://github.com/onlyone",
      "https://github.com/me/../../etc",
      "https://user:secret@github.com/me/furlpay-bot",
      "https://token@github.com/me/furlpay-bot",
      "ssh://bad%0auser@github.com/me/furlpay-bot",
      "https://github.com/me/furlpay-bot?x=1",
      "https://github.com/me/furlpay-bot#frag",
      "https://github.com/me/rak azo",
      "--upload-pack=touch /tmp/pwned",
      "-o ProxyCommand=id",
      `https://github.com/me/furlpay-bot${"x".repeat(400)}`,
    ]) {
  ...
  it("rejects an embedded newline that could smuggle a second argument", () => {
    expect(normalizeRepoUrl("https://github.com/me/furlpay-bot\n--exec=id")).toHaveProperty("error");
  });
});

describe("repoIdentity", () => {
  it("treats every spelling of the same remote as one repository", () => {
    // repoIdentity lowercases deliberately, so "FurlPay" and "furlpay" resolve
    // to one repository. The expected value is therefore the normalized form.
    const identity = "github.com/furlpay/furlpay-bot";
    expect(repoIdentity("https://github.com/FurlPay/furlpay-bot")).toBe(identity);
    expect(repoIdentity("https://github.com/FurlPay/furlpay-bot.git")).toBe(identity);
    expect(repoIdentity("git@github.com:FurlPay/furlpay-bot.git")).toBe(identity);
    // Mixed case must fold onto the same identity — that is the whole point of
    // normalizing, so a remote spelled with different capitalisation is not
    // mistaken for a different repository.
    expect(repoIdentity("ssh://git@github.com/FurlPay/FurlPay-Bot")).toBe(identity);
    expect(repoIdentity("not a url")).toBeNull();
  });

  it("only calls the real upstream official", () => {
    expect(isOfficialRepoUrl("git@github.com:FurlPay/furlpay-bot.git")).toBe(true);
    expect(isOfficialRepoUrl("https://github.com/attacker/furlpay-bot")).toBe(false);
    expect(isOfficialRepoUrl("https://githubb.com/FurlPay/furlpay-bot")).toBe(false);
  });
});
...
describe("updateSteps", () => {
  ...
  it("passes every value as its own argument so nothing reaches a shell", () => {
    const steps = updateSteps({
      remoteUrl: "git@github.com:me/furlpay-bot.git",
      branch: "release/1.2",
      targetCommit: "2".repeat(40),
      repointRemote: true,
    });
    expect(steps[0]).toEqual({
      ...
      command: "git",
      args: ["remote", "set-url", "origin", "git@github.com:me/furlpay-bot.git"],
    });

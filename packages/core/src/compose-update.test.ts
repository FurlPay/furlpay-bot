...
const target = {
  composeFile: "/srv/furlpay-bot/infra/compose/docker-compose.prod.yml",
  envFiles: ["/srv/furlpay-bot/.env"],
};
...
describe("image references", () => {
  ...
  it("accepts registry paths with a host and rejects anything else", () => {
    ...
    expect(isValidImageName("furlpay-bot/app")).toBe(true);
...
describe("compose argv construction", () => {
  ...
  it("defaults -p to the name pinned in the compose file", () => {
    expect(DEFAULT_COMPOSE_PROJECT_NAME).toBe("furlpay-bot-prod");
    expect(composePsArgv(target).args.slice(0, 3)).toEqual(["compose", "-p", "furlpay-bot-prod"]);
  });
  ...
  it("prefers the name Compose injects into running services", () => {
    ...
    expect(
      resolveComposeProjectName({
        ...
        FURLPAY_COMPOSE_PROJECT_NAME: "manual-stack",
      }),
    ).toBe("live-stack");
  });

  it("falls back to a dedicated override, then to the name pinned in the compose file", () => {
    expect(resolveComposeProjectName({ FURLPAY_COMPOSE_PROJECT_NAME: "manual-stack" })).toBe(
      "manual-stack",
    );
...
describe("update plans", () => {
  ...
  it("fetches, fast-forwards, then builds on the fork path", () => {
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl: "https://github.com/someone/furlpay-bot",
      branch: "trunk",
      repointRemote: true,
    });
  ...
  it("keeps the repository URL in argv rather than in any compose input", () => {
    const repoUrl = "https://github.com/someone/furlpay-bot";
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl,
...
describe("managed env assignments", () => {
  it("replaces a managed key in place and leaves everything else alone", () => {
    const contents = [
      ...
      "FURLPAY_IMAGE_TAG=v1.0.0",
      "",
    ].join("\n");
    expect(upsertEnvAssignments(contents, { FURLPAY_IMAGE_TAG: "v1.1.0" })).toBe(
      ["# deployment", "POSTGRES_PASSWORD=secret", "FURLPAY_IMAGE_TAG=v1.1.0", ""].join("\n"),
    );
  });

  it("appends keys that are not present yet", () => {
    const result = upsertEnvAssignments("POSTGRES_PASSWORD=secret\n", {
      FURLPAY_IMAGE_TAG: "v1.1.0",
  ...
  it("rewrites every duplicate managed assignment so the last value cannot win", () => {
    const result = upsertEnvAssignments(
      "FURLPAY_IMAGE_TAG=stale\nA=1\nFURLPAY_IMAGE_TAG=still-stale\n",
      { FURLPAY_IMAGE_TAG: "sha-0123456789abcdef0123456789abcdef01234567" },
    );
    expect(result.match(/FURLPAY_IMAGE_TAG=sha-/g)).toHaveLength(2);
  ...
  it("refuses values that would inject a second assignment or a compose expression", () => {
    ...
    for (const value of ["v1\nEVIL=1", "v1 v2", "$(id)", "v1;rm -rf /", "${OTHER}"]) {
      expect(() => upsertEnvAssignments("", { FURLPAY_IMAGE_TAG: value })).toThrow(/Refusing/);
    }
  });
...
describe("sidecar boundary validation", () => {
  it("normalizes a request the API already validated", () => {
    expect(
      validateUpdateRequest({ repoUrl: "https://github.com/FurlPay/furlpay-bot.git", branch: " main " }),
    ).toEqual({
      request: {
        repoUrl: "https://github.com/FurlPay/furlpay-bot.git",
        branch: "main",
        official: true,
      },
    });

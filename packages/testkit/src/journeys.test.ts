...
} from "@furlpay-bot/adapters";
import { ONCE_ROUTINE_CRON } from "@furlpay-bot/core";
import {
  appendEvent,
  createThreadEvents,
  createThreadMessage,
...
describeJourneys("required product journeys", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-journey-"));
  ...
  it("1+2: users are isolated and workspace bots share the Team Computer", async () => {
    const ada = await signup(app, `ada-j-${stamp}@furlpay-bot.test`, "Ada Journey");
    const bob = await signup(app, `bob-j-${stamp}@furlpay-bot.test`, "Bob Journey");
  ...
  it("clears a conversation without removing the bot, computer, memory, or routines", async () => {
    const cookie = await signup(app, `clear-j-${stamp}@furlpay-bot.test`, "Clear Journey");
  ...
  it("2b: two Team bots send at once on distinct screens", async () => {
    const cookie = await signup(app, `parallel-j-${stamp}@furlpay-bot.test`, "Parallel");
  ...
  it("3: disconnect and reconnect from a cursor reconstructs the thread", async () => {
    const cookie = await signup(app, `cursor-j-${stamp}@furlpay-bot.test`, "Cursor");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("4: takeover login then resume without exposing credentials", async () => {
    const cookie = await signup(app, `takeover-j-${stamp}@furlpay-bot.test`, "Takeover");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("4d: skipping takeover resumes without treating login as done", async () => {
    const cookie = await signup(app, `takeover-skip-j-${stamp}@furlpay-bot.test`, "Skip Takeover");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("4b: an expired takeover denies input and reconciles API and database state", async () => {
    ...
    try {
      const cookie = await signup(app, `takeover-expiry-j-${stamp}@furlpay-bot.test`, "Expiry");
      const bot = await rpc<Bot>(app, cookie, "bots/create", {
        name: "Chief",
        title: "",
        description: "",
  ...
  it("4c: a takeover authorizes input only on the controlled bot screen", async () => {
    const cookie = await signup(app, `takeover-scope-j-${stamp}@furlpay-bot.test`, "Takeover Scope");
    const writer = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Writer",
      title: "",
      description: "",
  ...
  it("4d: a stale Team release cannot clear a newer bot takeover", async () => {
    const cookie = await signup(
      app,
      `takeover-release-fence-j-${stamp}@furlpay-bot.test`,
      "Release Fence",
    );
    const writer = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Writer",
  ...
  it("5: a routine wakes the bot and posts into the existing thread", async () => {
    const cookie = await signup(app, `routine-j-${stamp}@furlpay-bot.test`, "Routine");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("5b: tool-created schedules wake in the creating group or 1:1 thread", async () => {
    const cookie = await signup(app, `schedule-dest-j-${stamp}@furlpay-bot.test`, "Schedule Dest");
  ...
  it("allocates event and message cursors atomically under concurrent writes", async () => {
    const cookie = await signup(app, `sequence-j-${stamp}@furlpay-bot.test`, "Sequence");
  ...
  it("7: destination write is independently inspectable and credentials stay out of the thread", async () => {
    const cookie = await signup(app, `dest-j-${stamp}@furlpay-bot.test`, "Dest");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("8: retrying a completed effect does not duplicate the destination write", async () => {
    const cookie = await signup(app, `crash-j-${stamp}@furlpay-bot.test`, "Crash");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("9: export includes memory and files but not secrets or browser sessions", async () => {
    const cookie = await signup(app, `export-j-${stamp}@furlpay-bot.test`, "Export");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("10: bots can be archived safely and deleted with or without their memories", async () => {
    const ada = await signup(app, `delete-j-${stamp}@furlpay-bot.test`, "Delete Ada");
    const bob = await signup(app, `delete-bob-j-${stamp}@furlpay-bot.test`, "Delete Bob");
  ...
  it("11: deleting an account removes the user and personal workspace data", async () => {
    const email = `account-delete-j-${stamp}@furlpay-bot.test`;
  ...
  it("12: a bot can spawn a regular bot and must confirm the name to delete it", async () => {
    const cookie = await signup(app, `spawn-j-${stamp}@furlpay-bot.test`, "Spawn");
  ...
  it("13: a subagent shows up in the parent thread without creating a bot", async () => {
    const cookie = await signup(app, `subagent-j-${stamp}@furlpay-bot.test`, "Subagent");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("14: this-mac is refused unless the sandbox is docker", async () => {
    const cookie = await signup(app, `host-j-${stamp}@furlpay-bot.test`, "Host");
    const me = await rpc<Me>(app, cookie, "me");
  ...
  it("15: ask, answer, stop, follow-up, and clientNonce stay consistent", async () => {
    const cookie = await signup(app, `ask-j-${stamp}@furlpay-bot.test`, "Ask");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("16: routine test-run and plugin connect/revoke", async () => {
    const ada = await signup(app, `plug-j-${stamp}@furlpay-bot.test`, "Plug Ada");
    const bob = await signup(app, `plug-bob-j-${stamp}@furlpay-bot.test`, "Plug Bob");
  ...
  it("54: group chats share one transcript with mentions and handoffs", async () => {
    const ada = await signup(app, `ada-g-${stamp}@furlpay-bot.test`, "Ada Groups");
    const adaMe = await rpc<Me>(app, ada, "me");
  ...
  it("17: teach a task end to end", async () => {
    const cookie = await signup(app, `teach-j-${stamp}@furlpay-bot.test`, "Teach Ada");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Teacher",
      title: "",
      description: "",
  ...
  it("18: teaching expiry auto-stops recording", async () => {
    ...
    try {
      const cookie = await signup(app, `teach-exp-j-${stamp}@furlpay-bot.test`, "Teach Exp Ada");
      const bot = await rpc<Bot>(app, cookie, "bots/create", {
        name: "Timer",
        title: "",
        description: "",
  ...
  it("19: destination writes pause for approval before side effects", async () => {
    const cookie = await signup(app, `approval-j-${stamp}@furlpay-bot.test`, "Approval");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("20: actions run by default and specific exceptions override broad review rules", async () => {
    const cookie = await signup(app, `always-j-${stamp}@furlpay-bot.test`, "Always");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("21: routine destination writes pause on the same approval card", async () => {
    const cookie = await signup(app, `routine-approval-j-${stamp}@furlpay-bot.test`, "Routine Approval");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("22: a routine schedule with any malformed or mixed one-shot cron is rejected", async () => {
    const cookie = await signup(app, `routine-crons-j-${stamp}@furlpay-bot.test`, "Routine Crons");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Scheduler",
      title: "",
      description: "",
  ...
  it("23: never-run one-shot templates can be armed with a future runAt", async () => {
    const cookie = await signup(app, `once-arm-j-${stamp}@furlpay-bot.test`, "Once Arm");
    const me = await rpc<Me>(app, cookie, "me");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Once Bot",
      title: "",
  ...
  it("24: chat creates a space only after explicit approval", async () => {
    const cookie = await signup(app, `space-chat-j-${stamp}@furlpay-bot.test`, "Space Chat");
    const me = await rpc<Me>(app, cookie, "me");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",

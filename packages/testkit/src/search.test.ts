...
describeSearch("workspace search", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-search-"));
  ...
  it("finds bots, messages, files, links, and routines within the workspace", async () => {
    const cookie = await signup(app, `search-${stamp}@furlpay-bot.test`, "Search User");
  ...
  it("returns no hits for another workspace", async () => {
    const ownerCookie = await signup(app, `search-owner-${stamp}@furlpay-bot.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `search-intruder-${stamp}@furlpay-bot.test`, "Intruder");
  ...
  it("finds group conversations, messages, and files", async () => {
    const cookie = await signup(app, `search-group-${stamp}@furlpay-bot.test`, "Group Search User");

...
import { ComposioEmulator } from "@furlpay-bot/adapters";
import type { appContract, Space, SpaceNavigation } from "@furlpay-bot/contracts";
...
describeWithDatabase("API authorization and resource isolation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-authz-"));
  ...
  it("prevents one user from reading or mutating another user's resources", async () => {
    const owner = await signup(app, `owner-authz-${stamp}@furlpay-bot.test`, "Authorization Owner");
    const intruder = await signup(
      app,
      `intruder-authz-${stamp}@furlpay-bot.test`,
  ...
  it("keeps approval rules private to each user in a shared Space", async () => {
    const owner = await signup(app, `approval-owner-${stamp}@furlpay-bot.test`, "Approval Owner");
    const member = await signup(app, `approval-member-${stamp}@furlpay-bot.test`, "Approval Member");
    const ownerActor = await rpc<Actor>(app, owner, "me");
  ...
  it("keeps space data and computers behind the selected space boundary", async () => {
    const cookie = await signup(app, `spaces-${stamp}@furlpay-bot.test`, "Space Owner");
    ...
    const intruder = await signup(app, `spaces-intruder-${stamp}@furlpay-bot.test`, "Intruder");
  ...
  it("enforces the space limit across concurrent creation requests", async () => {
    const cookie = await signup(app, `space-limit-${stamp}@furlpay-bot.test`, "Space Limit");
  ...
  it("reuses provider credentials and copies their selections into a new Space", async () => {
    const cookie = await signup(app, `space-provider-copy-${stamp}@furlpay-bot.test`, "Provider Copy");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("shares model credentials while keeping defaults private to each space", async () => {
    const cookie = await signup(app, `model-defaults-${stamp}@furlpay-bot.test`, "Model Defaults");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("validates per-bot model overrides against connected providers and catalog", async () => {
    const cookie = await signup(app, `bot-model-${stamp}@furlpay-bot.test`, "Bot Model");
    const bot = await rpc<
      Bot & {
  ...
  it("chooses the newest duplicate provider credential when selecting a default", async () => {
    const cookie = await signup(app, `model-duplicates-${stamp}@furlpay-bot.test`, "Model Duplicates");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("restricts deployment settings to the deployment owner", async () => {
    const owner = await signup(app, `deployment-owner-${stamp}@furlpay-bot.test`, "Deployment Owner");
    const other = await signup(app, `deployment-other-${stamp}@furlpay-bot.test`, "Deployment Other");
    const ownerActor = await rpc<Actor>(app, owner, "me");
    ...
    try {
      ...
      const closedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        body: JSON.stringify({
          email: `closed-${stamp}@furlpay-bot.test`,
      ...
      const disallowedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          email: `not-approved-${stamp}@furlpay-bot.test`,
          password: "password123",
...
async function raw(
  ...
  return app.request(`/rpc/${procedure}`, {
    ...
    headers: {
      ...
      ...(cookie ? { cookie } : {}),
      ...(spaceId ? { "x-furlpay-bot-space-id": spaceId } : {}),

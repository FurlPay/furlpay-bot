...
import type { RunActivityRow } from "@furlpay-bot/contracts";
import type { PrismaClient } from "@furlpay-bot/db";
...
describeRunsList("runs.list activity tracker", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-runs-list-"));
  ...
  it("lists active runs from two bots and keeps completed runs under recent", async () => {
    const cookie = await signup(app, `runs-${stamp}@furlpay-bot.test`, "Runs User");
  ...
  it("never returns runs from another workspace", async () => {
    const ownerCookie = await signup(app, `runs-owner-${stamp}@furlpay-bot.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `runs-intruder-${stamp}@furlpay-bot.test`, "Intruder");
  ...
  it("excludes archived bots from recent before applying the limit", async () => {
    const cookie = await signup(app, `runs-archived-${stamp}@furlpay-bot.test`, "Archive User");
  ...
  it("returns group runs with groupId for navigation", async () => {
    const cookie = await signup(app, `runs-group-${stamp}@furlpay-bot.test`, "Group User");

...
import { createThreadEvents } from "@furlpay-bot/db";
...
describeIntegration("run executor lifecycle", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-executor-lifecycle-"));
  ...
  it("records an uncertain result without replaying an interrupted external effect", async () => {
    ...
    const args = { collection: "notes", title: "FurlPay Bot result", body: prompt };
  ...
  it("applies the same no-parallel-run rule to the targeted group member", async () => {
    const cookie = await signup(
      `executor-group-steering-${stamp}@furlpay-bot.test`,
  ...
  async function seedRun(
    ...
    const cookie = await signup(`executor-${label}-${stamp}@furlpay-bot.test`, `Executor ${label}`);
    const me = await rpc<{ userId: string; spaceId: string }>(cookie, "me");

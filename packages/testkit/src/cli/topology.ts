...
import type { ThreadSnapshot } from "@furlpay-bot/contracts";
import { isTerminal } from "@furlpay-bot/core";

const composeFile = path.resolve("infra/compose/docker-compose.topology.yml");
const keep = process.argv.includes("--keep");
const skipBuild = process.argv.includes("--skip-build");
const project = `furlpay-bot-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "furlpay-bot/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "furlpay-bot/computer:topology",
  };
...
async function signup(baseUrl: string) {
  const response = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({
      email: `topology-${Date.now()}@furlpay-bot.test`,
      password: "password12",
      name: "Topology",
    }),
  });
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (logs.includes("furlpay-bot worker ready")) return;
    await delay(500);
  }
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "furlpay-bot.managed"}}',
  ]).trim();
  ...
  try {
    ...
    const ids = isolatedSupervisorNetworks(supervisorId, projectName).flatMap((networkName) =>
      docker([
        ...
        `network=${networkName}`,
        "--filter",
        "label=furlpay-bot.managed=true",
      ])
        .split("\n")
        .map((id) => id.trim())
        .filter(Boolean),

...
import { abortableDelay } from "@furlpay-bot/core";
import { loadRootEnv } from "@furlpay-bot/core/node/load-root-env";
import { createThreadMessage, type PrismaClient } from "@furlpay-bot/db";
...
const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "furlpay-bot-desktop-performance-"));
...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "furlpay-bot-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "furlpay-bot-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "furlpay-bot-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "furlpay-bot-benchmark-screen-proxy-secret-over-32-characters",
    ...
    FURLPAY_HOST: "127.0.0.1",
    FURLPAY_WEB_URL: webOrigin,
    FURLPAY_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    FURLPAY_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
    FURLPAY_PERFORMANCE_ASSET_DELAY_MS: String(assetDelayMs),
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@furlpay-bot/desktop", "pack:dir"], env);
}

function migrateDatabase(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@furlpay-bot/db", "generate"], env);
  run("pnpm", ["--filter", "@furlpay-bot/db", "exec", "prisma", "migrate", "deploy"], env);
}
...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "FurlPay Bot.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/FurlPay Bot");
  }
...
async function prepareAuthenticatedProfile(benchmark: BenchmarkContext, profile: string) {
  ...
  try {
    ...
    await page.getByPlaceholder("Your email address").fill(`benchmark-${stamp}@furlpay-bot.test`);
    ...
    if (await createHeading.isVisible().catch(() => false)) {
      ...
      await page.getByRole("button", { name: "Open FurlPay Bot" }).click();
    }
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      FURLPAY_PERFORMANCE_USER_DATA: profile,
      FURLPAY_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
...
async function measureInteractions(app: ElectronApplication, page: Page) {
  ...
  await page.evaluate(() => {
    ...
    (window as typeof window & { __furlpayBotKeyPaintSamples?: number[] }).__furlpayBotKeyPaintSamples =
      samples;
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "FurlPay Bot.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# FurlPay Bot desktop performance — ${report.label}

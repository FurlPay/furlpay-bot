...
async function main() {
  ...
  try {
    ...
    execSync("pnpm --filter @furlpay-bot/db generate", { stdio: "inherit", env: process.env });
    execSync("pnpm --filter @furlpay-bot/db exec prisma migrate deploy", {
      stdio: "inherit",
      env: process.env,
    ...
    const [
      ...
      { createApp },
    ] = await Promise.all([import("@furlpay-bot/adapters"), import("../../../../apps/api/src/app.ts")]);

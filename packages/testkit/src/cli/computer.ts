...
async function main() {
  ...
  const dataDir = await mkdtemp(path.join(tmpdir(), "furlpay-bot-computer-e2e-run-"));
  ...
  try {
    execFileSync("pnpm", ["--filter", "@furlpay-bot/db", "generate"], {
      stdio: "inherit",
      env,
    });
    execFileSync("pnpm", ["--filter", "@furlpay-bot/db", "exec", "prisma", "migrate", "deploy"], {
      stdio: "inherit",
      env,

...
import { SCRIPTED_MPEG, SCRIPTED_TRANSCRIPT, SCRIPTED_VOICE_ID } from "@furlpay-bot/adapters";
...
describeVoice("voice credentials and speech HTTP", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-voice-"));
  ...
  it("connects a scripted key, speaks, and transcribes without leaking the secret", async () => {
    const cookie = await signup(app, `voice-${stamp}@furlpay-bot.test`, "Voice User");
    const other = await signup(app, `voice-other-${stamp}@furlpay-bot.test`, "Other Voice");

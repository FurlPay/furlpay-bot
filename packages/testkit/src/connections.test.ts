...
describeWithDatabase("Composio catalog reconciliation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "furlpay-bot-connections-"));
  ...
  it("reconciles one scoped row per provider under concurrent catalog fetches", async () => {
    const ownerCookie = await signup(app, `owner-connections-${stamp}@furlpay-bot.test`, "Owner");
    const otherCookie = await signup(app, `other-connections-${stamp}@furlpay-bot.test`, "Other");
  ...
  it("returns the remote catalog when local reconciliation fails", async () => {
    const cookie = await signup(app, `db-failure-connections-${stamp}@furlpay-bot.test`, "DB Failure");
  ...
  it("does not mutate local state when the provider catalog fails", async () => {
    const cookie = await signup(
      app,
      `provider-failure-connections-${stamp}@furlpay-bot.test`,
      "Provider Failure",
    );
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("routes an emulated Composio app tool with user-scoped connection context", async () => {
    const cookie = await signup(app, `composio-tool-${stamp}@furlpay-bot.test`, "Composio Tool");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("runs Pipedream connection and MCP tool execution through the product registry", async () => {
    const cookie = await signup(app, `pipedream-${stamp}@furlpay-bot.test`, "Pipedream Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("installs Treg and custom MCP sources, discovers tools, and routes both calls", async () => {
    const cookie = await signup(app, `mcp-connectors-${stamp}@furlpay-bot.test`, "MCP Connectors");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("imports an OpenAPI connector, keeps its credential encrypted, and routes calls", async () => {
    const cookie = await signup(app, `api-connector-${stamp}@furlpay-bot.test`, "API Connector");
    const actor = await rpc<Actor>(app, cookie, "me");

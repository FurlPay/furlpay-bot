...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# FurlPay Bot");
    expect(getMarkdownDocument("/about/")).toContain("# About FurlPay Bot");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use FurlPay Bot");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use FurlPay Bot");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# FurlPay Bot\n");
    ...
    await expect(response.text()).resolves.toBe("# FurlPay Bot\n");

    const headResponse = markdownResponse("# FurlPay Bot\n", "HEAD", 404);

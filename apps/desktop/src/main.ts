...
import type { DesktopReachability, DesktopSetup } from "@furlpay-bot/contracts";
...
const PERFORMANCE_USER_DATA = process.env.FURLPAY_PERFORMANCE_USER_DATA;
...
const WARM_WINDOW_TTL_MS = warmWindowTtlMs(process.env.FURLPAY_WARM_WINDOW_TTL_MS);

const updaterEnvironment = {
  packaged: app.isPackaged,
  version: app.getVersion(),
  disabled: process.env.FURLPAY_DISABLE_AUTO_UPDATE === "1",
};
...
function createWindow(url: string, partition: string | null) {
  ...
  win.on("close", (event) => {
    if (
      ...
      !quitting &&
      process.env.FURLPAY_DISABLE_WARM_WINDOW !== "1"
    ) {
      event.preventDefault();
      win.hide();
 ...
 * fixtures omit the FurlPay Bot app-state marker.
 */
async function waitForMountedAppDocument(contents: Electron.WebContents) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await contents.executeJavaScript(`(() => {
      const appState =
        document.querySelector("[data-furlpay-bot-app-state]")?.getAttribute("data-furlpay-bot-app-state") ??
        null;
      ...
      // Desktop e2e fixtures mount a plain page without FurlPay Bot app-state markers.
      if (appState === null) {
...
async function installBundledRenderer(
  ...
  if (!app.isPackaged || process.env.FURLPAY_DISABLE_BUNDLED_RENDERER === "1") return;
...
function installApplicationMenu() {
  const changeServer: Electron.MenuItemConstructorOptions = {
    id: "change-furlpay-bot-server",
    label: "Change FurlPay Bot Server…",
...
/** Setup IPC must only answer the setup window, never a connected FurlPay Bot server. */
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return {
        ...
        url,
        error: "That address redirects elsewhere. Enter the final FurlPay Bot server address.",
      };
    }
    if (!response.ok) {
      return {
    ...
    if (!isFurlPayBotHealth(health)) {
      return {
        ok: false,
        status: response.status,
        url,
        error: "That address did not respond like a FurlPay Bot server.",
      };
    }
    return {
      ok: true,
...
app.whenReady().then(async () => {
  ...
  const target = resolveStartupTarget({
    envUrl: process.env.FURLPAY_WEB_URL,
    saved: currentSetup,
    forceSetup: process.env.FURLPAY_FORCE_SETUP === "1",
  });
  if (process.env.FURLPAY_PERFORMANCE_CLEAR_CACHE === "1") {

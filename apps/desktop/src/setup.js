(() => {
  const bridge = window.furlpayBotSetup;
  ...
  async function check() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus(`FurlPay Bot answered at ${result.url}.`, "ok");
      } else {
  ...
  async function init() {
    ...
    try {
      ...
      setStatus("Setup could not start. Quit FurlPay Bot and try again.", "error");
      setBusy(true);
    }
  }

...
describe("server address normalization", () => {
  it("assumes http locally and https for a bare public host", () => {
    ...
    expect(normalizeServerUrl("furlpay-bot.example.com")).toBe("https://furlpay-bot.example.com");
  });

  it("keeps an explicit secure scheme and port but stores only the origin", () => {
    expect(normalizeServerUrl("https://furlpay-bot.example.com")).toBe("https://furlpay-bot.example.com");
    expect(normalizeServerUrl("https://furlpay-bot.example.com:8443/team")).toBe(
      "https://furlpay-bot.example.com:8443",
    );
  });
  ...
  it("rejects cleartext public servers but permits private-network development", () => {
    expect(normalizeServerUrl("http://furlpay-bot.example.com")).toBeNull();
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match FurlPay Bot.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a FurlPay Bot server (%s)",
    (value) => {
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@furlpay-bot.example.com")).toBeNull();
  });
});

describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://furlpay-bot.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://furlpay-bot.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://furlpay-bot.example.com",
      source: "saved",
    });
  });

  it("lets FURLPAY_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty FURLPAY_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://furlpay-bot.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:furlpay-bot-[a-f0-9]{24}$/);
...
describe("FurlPay Bot health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isFurlPayBotHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isFurlPayBotHealth({ json: { ok: true } })).toBe(false);
    expect(isFurlPayBotHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});

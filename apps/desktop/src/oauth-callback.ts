import type { FurlPayBotDesktopOAuthCallback } from "@furlpay-bot/contracts";
 ...
 * in the redirect URL. FurlPay Bot runs no listener on that port and asks for the
...
): FurlPayBotDesktopOAuthCallback | undefined {
  let target: URL;
  try {
    target = new URL(url);
  } catch {

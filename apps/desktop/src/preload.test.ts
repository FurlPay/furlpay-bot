...
import type { FurlPayBotDesktop, FurlPayBotSetup } from "@furlpay-bot/contracts";
...
describe("desktop preload bridge", () => {
  it("exposes only the platform, the four window operations, the updater, and the OAuth bridge", async () => {
    ...
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, FurlPayBotDesktop];
    expect(globalName).toBe("furlpayBotDesktop");
  ...
  it("forwards captured codes without leaking the IPC event to the renderer", () => {
    ...
    const [, bridge] = exposeInMainWorld.mock.calls[0] as [string, FurlPayBotDesktop];
...
describe("setup preload bridge", () => {
  it("exposes only the first-run setup operations", async () => {
    ...
    expect(exposeInMainWorld).toHaveBeenCalledTimes(1);
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, FurlPayBotSetup];
    expect(globalName).toBe("furlpayBotSetup");

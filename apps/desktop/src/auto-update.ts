import type { DesktopUpdateState } from "@furlpay-bot/contracts";
...
export function classifyUpdaterFailure(error: unknown): UpdaterFailure {
  ...
  if (SIGNATURE.some((needle) => text.includes(needle))) {
    return {
      kind: "signature",
      message: "This update could not be verified. Reinstall FurlPay Bot from a trusted download.",
    };
  }
  return {
    kind: "other",
  ...
  switch (event.type) {
    ...
    case "downloaded":
      return {
        ...
        availableVersion: event.version,
        percent: 100,
        message: "Restart FurlPay Bot to finish the update.",
      };
    case "failed": {

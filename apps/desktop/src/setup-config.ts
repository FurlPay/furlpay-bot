...
import type { DesktopSetup } from "@furlpay-bot/contracts";

/** Where `pnpm dev` serves the FurlPay Bot web app on this machine. */
...
/**
 * Accepts what a person would actually type ("localhost:5173", "furlpay-bot.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a FurlPay Bot server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // FurlPay Bot serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `FURLPAY_WEB_URL` still wins over saved configuration so test and performance
...
/** Each FurlPay Bot origin gets its own persistent cookie and storage partition. */
export function sessionPartitionForServerUrl(targetUrl: string): string | null {
  try {
    ...
    return `persist:furlpay-bot-${digest}`;
  } catch {
    return null;
  }
}
...
export function isFurlPayBotHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate FurlPay Bot

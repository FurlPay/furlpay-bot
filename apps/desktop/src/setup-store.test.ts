...
beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "furlpay-bot-setup-"));
});
...
describe("setup store", () => {
  ...
  it("keeps the chosen instance across launches", async () => {
    await writeSetup(userData, { mode: "existing", serverUrl: "https://furlpay-bot.example.com" });
    await expect(readSetup(userData)).resolves.toEqual({
      mode: "existing",
      serverUrl: "https://furlpay-bot.example.com",
    });
  });

...
import type { FurlPayBotDesktop } from "@furlpay-bot/contracts";

const fixture = `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8"><title>FurlPay Bot desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  const app = await electron.launch({
    ...
    env: {
      ...process.env,
      FURLPAY_WEB_URL: `data:text/html;charset=utf-8,${encodeURIComponent(fixture)}`,
    },
  });
  ...
  try {
    ...
    await expect(page).toHaveTitle("FurlPay Bot desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { furlpayBotDesktop?: FurlPayBotDesktop }).furlpayBotDesktop;

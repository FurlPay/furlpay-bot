...
const APP_MARKER = "Existing FurlPay Bot instance ready";
...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.end(
      `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FurlPay Bot</title></head><body><main>${APP_MARKER}</main></body></html>`,
    );
  });
...
test.beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "furlpay-bot-desktop-e2e-"));
});
...
function launch(extraEnv: Record<string, string> = {}) {
  const env = { ...process.env, FURLPAY_PERFORMANCE_USER_DATA: userData };
  // A stale FURLPAY_WEB_URL from the developer's shell would bypass setup entirely.
  delete env.FURLPAY_WEB_URL;
...
test("first run asks whether to use a local or existing instance", async () => {
  ...
  await expect(setup.getByRole("heading", { name: "Welcome to FurlPay Bot" })).toBeVisible();
...
test("connecting to an existing instance verifies, saves, and opens it", async () => {
  ...
  await expect(setup.locator("#status")).toHaveText(`FurlPay Bot answered at ${serverUrl}.`);
...
test("a session-pending shell skeleton is not accepted as a ready app", async () => {
  const skeletonHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FurlPay Bot</title></head>
<body><div id="root"><div data-furlpay-bot-app-state="session-pending"><aside></aside><main><div>Opening your Space…</div></main></div></div></body></html>`;
...
test("a post-session ready app mount is accepted", async () => {
  const readyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FurlPay Bot</title>
<script>performance.mark("rk:renderer:session-committed");performance.mark("rk:renderer:shell-ready");</script>
</head>
<body><div id="root"><div data-furlpay-bot-app-state="ready"><div data-testid="shell-root" data-ready="true">Workspace</div></div></div></body></html>`;
...
test("a shell mount before workspace bootstrap is not accepted", async () => {
  const preBootstrapHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FurlPay Bot</title></head>
<body><div id="root"><div data-furlpay-bot-app-state="ready"><div data-testid="shell-root" data-ready="false">Workspace</div></div></div></body></html>`;
...
test("a session-ready marker without a route surface is not accepted", async () => {
  const emptyReadyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FurlPay Bot</title></head>
<body><div id="root"><div data-furlpay-bot-app-state="ready" class="h-full"></div></div></body></html>`;
...
test("a generic web page is not accepted as a FurlPay Bot server", async () => {
  const plain = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end("<!doctype html><p>not FurlPay Bot</p>");
  });
  ...
  try {
    ...
    await expect(setup.locator("#status")).toHaveText(
      "That address did not respond like a FurlPay Bot server.",
    );
    await expect(async () => {
      await expect(readFile(path.join(userData, "setup.json"), "utf8")).rejects.toThrow();
    }).toPass();
...
test("the native application menu can reopen setup without exposing setup IPC to the server", async () => {
  app = await launch({ FURLPAY_WEB_URL: serverUrl });
  const appWindow = await app.firstWindow();
  await expect(appWindow.getByText(APP_MARKER)).toBeVisible();

  const setupPromise = app.waitForEvent("window");
  await app.evaluate(({ Menu }) => {
    const item = Menu.getApplicationMenu()?.getMenuItemById("change-furlpay-bot-server");
...
test("servers on the same host but different ports do not share login cookies", async () => {
  const first = createServer((_request, response) => {
    response.writeHead(200, {
      ...
      "set-cookie": "furlpayBot_session=fake-one; Path=/; SameSite=Lax",
    });
  ...
  try {
    app = await launch({ FURLPAY_WEB_URL: `http://127.0.0.1:${firstAddress.port}` });
    ...
    app = await launch({ FURLPAY_WEB_URL: `http://127.0.0.1:${secondAddress.port}` });
...
test("setup IPC is not reachable from the connected app window", async () => {
  ...
  const exposed = await appWindow.evaluate(() =>
    Object.keys((window as typeof window & { furlpayBotSetup?: unknown }).furlpayBotSetup ?? {}),
  );

...
export default defineConfig(({ mode }) => {
  ...
  const previewHost = process.env.FURLPAY_HOST ?? rootEnv.FURLPAY_HOST ?? "localhost";
  ...
  const performanceAssetDelayMs = Number(process.env.FURLPAY_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  return {
    plugins: [
      react({
        babel: {
      ...
      tailwindcss(),
      {
        name: "furlpay-bot-performance-asset-delay",

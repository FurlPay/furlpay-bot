...
export function ArtifactFileCard(props: ArtifactFileCardProps) {
    ...
    return (
      <div>
        <button
          ...
          className="rounded-[20px] border border-[var(--fp-border)] bg-[var(--fp-hairline)] px-4 py-3 text-left text-[14px] text-[var(--fp-body)] hover:bg-[var(--fp-elevated)]"
        >
          <div className="font-medium">{props.name}</div>
          <div className="mt-1 text-[var(--fp-muted)]">
  ...
  return (
    <>
      <div>
        <div className="flex min-w-[280px] overflow-hidden rounded-[20px] border border-[var(--fp-border)] bg-[var(--fp-elevated)] text-left text-[var(--fp-body)]">
          <button
            ref={previewButton}
            type="button"
            aria-label={t`Preview ${props.name}`}
            onClick={() => setPreviewOpen(true)}
            className="flex min-w-0 flex-1 items-center gap-3 px-4 py-3 text-left hover:bg-[var(--fp-scroll)]"
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-0.5 block text-[13px] text-[var(--fp-muted)]">
          ...
          <button
            ...
            onClick={() => void startDownload()}
            className="grid w-14 shrink-0 place-items-center border-l border-[var(--fp-elevated)] text-[var(--fp-muted)] hover:bg-[var(--fp-scroll)] hover:text-[var(--fp-ink)]"
          >
      ...
      >
        <header className="flex h-14 shrink-0 items-center border-b border-[#27272B] px-5">
          ...
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-full text-[#929298] hover:bg-[var(--fp-elevated)] hover:text-[var(--fp-ink)]"
          >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <article className="mx-auto w-full max-w-[760px] px-8 py-10 text-[16px] leading-7 text-[#D5D5D8] sm:px-12 sm:py-12">
            {state.status === "loading" ? (
              <div className="text-[var(--fp-muted)]">
                <Trans>Loading preview…</Trans>
              </div>
            ) : state.status === "error" ? (
              <div className="rounded-[14px] border border-[#5A2A2A] bg-[#2A1717] px-4 py-3 text-[var(--fp-danger-soft)]">
                {state.message}
              </div>
            ) : (
...
function DownloadError({ message }: { message: string }) {
  return (
    <div role="alert" className="mt-2 text-left text-[13px] text-[var(--fp-danger)]">
      {message}
    </div>
  );
}

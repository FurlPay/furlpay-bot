...
import type { ComputerStatus } from "@furlpay-bot/contracts";
    ...
    <div className="relative flex max-w-[min(360px,100%)] flex-col items-end">
      {goalOpen ? (
        <div
          ...
          className="absolute end-0 top-full z-20 mt-2 w-[min(360px,calc(100vw-2rem))] rounded-[12px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3 py-3 shadow-[0_12px_40px_rgba(0,0,0,.45)]"
        >
          <label htmlFor="teach-goal-input" className="text-[13px] text-[var(--fp-muted)]">
          ...
          <textarea
            ...
            rows={3}
            className="mt-2 w-full rounded-[10px] border border-[var(--fp-border)] bg-[var(--fp-panel)] px-3 py-2 text-[14px] text-[var(--fp-ink)] outline-none"
            placeholder={t`Export this week's list from the CRM and drop it in the shared folder`}
          />
          {error ? (
            <div role="alert" className="mt-2 text-[13px] text-[var(--fp-danger-soft)]">
              {error}
            </div>
          ) : null}
          <div className="mt-3 flex gap-2">
            <button
              ...
              className="rounded-[11px] bg-[var(--fp-cream)] px-4 py-2 text-[14px] text-[var(--fp-cream-ink)] disabled:opacity-40"
            >
            ...
            <button
              ...
              className="rounded-[11px] border border-[var(--fp-border)] px-4 py-2 text-[14px] text-[var(--fp-ink)]"
            >
              <Trans>Cancel</Trans>
            </button>
          </div>
      ...
      {!goalOpen && needsRefresh && recoveryOpen ? (
        <div
          ...
          className="absolute end-0 top-full z-20 mt-2 w-[min(320px,calc(100vw-2rem))] rounded-[12px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3 py-3 shadow-[0_12px_40px_rgba(0,0,0,.45)]"
        >
          {error ? (
            <div role="alert" className="text-[13px] text-[var(--fp-danger-soft)]">
              {error}
            </div>
          ) : (
      ...
      <button
        ...
        className="flex items-center gap-2 rounded-[10px] border border-[var(--fp-scroll)] bg-[var(--fp-surface)] px-3 py-1.5 text-[13px] text-[var(--fp-ink)] hover:bg-[var(--fp-surface-2)] disabled:opacity-40"
      >
        <span
          aria-hidden
          className="inline-block h-2 w-2 shrink-0 rounded-full border border-[var(--fp-ink)]"
        />

...
import type { SkillPlaybook } from "@furlpay-bot/contracts";
import { formatSkillRunPrompt } from "@furlpay-bot/core";
...
function fieldLabel(id: string, title: React.ReactNode) {
  return (
    <label htmlFor={id} className="mt-3 block text-[13px] text-[var(--fp-muted)]">
      {title}
    </label>
  );
}
...
function fieldClassName() {
  return "mt-1 w-full rounded-[10px] border border-[var(--fp-border)] bg-[var(--fp-panel)] px-3 py-2 text-[14px] text-[var(--fp-ink)] outline-none";
}
...
export function SkillDraftCard({
  ...
  return (
    <div
      ...
      className="w-[min(520px,92%)] rounded-[20px] border border-[var(--fp-border)] bg-[var(--fp-surface)] px-[18px] py-4"
    >
      <div className="text-[15px] font-medium text-[var(--fp-ink)]">
        <Trans>Draft skill</Trans>
      </div>
      <div className="mt-1 text-[13.5px] text-[var(--fp-muted)]">{block.goal}</div>

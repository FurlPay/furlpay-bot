...
export function ComputerMaintenanceActions({
  ...
  const resetDialog = confirmReset ? (
    ...
    >
      <BuiCard className="w-full max-w-[420px] border border-[var(--fp-hairline-strong)] p-5">
        <div id="reset-computer-title" className="text-[16px] font-medium text-[var(--fp-ink)]">
          <Trans>Reset computer?</Trans>
        </div>
        <p
          id="reset-computer-description"
          className="mt-2 text-[14px] leading-[1.5] text-[var(--fp-muted)]"
        >
    ...
    return (
      <div ref={rootRef} className="relative">
        <button
          ...
          className="grid h-8 w-8 place-items-center rounded-[10px] text-[var(--fp-muted)] hover:bg-[var(--fp-elevated)] hover:text-[var(--fp-ink)] disabled:opacity-40"
        >
        ...
        {menuOpen ? (
          <div
            ...
            className="absolute end-0 top-full z-20 mt-1.5 min-w-[180px] rounded-[12px] border border-[var(--fp-scroll)] bg-[var(--fp-surface)] py-1.5 shadow-[0_12px_40px_rgba(0,0,0,.55)]"
          >
            {showRecover ? (
              <button
                type="button"
                ...
                className="flex w-full px-3.5 py-2.5 text-start text-[14px] text-[var(--fp-ink)] hover:bg-[var(--fp-elevated)] disabled:opacity-40"
              >
                {pending === "recover" ? (
            ...
            {error ? (
              <p className="px-3.5 py-2 text-[12.5px] text-[var(--fp-danger)]">{error}</p>
            ) : null}
          </div>
        ) : null}
        {resetDialog}
  ...
  return (
    <div className={compact ? "flex flex-col items-start gap-2" : "mt-4 flex flex-col gap-3"}>
      ...
      {!compact ? (
        <p className="text-[13px] leading-[1.45] text-[var(--fp-muted-2)]">
          <Trans>
      ...
      {error && !confirmReset ? (
        <p className="text-[13px] text-[var(--fp-danger)]">{error}</p>
      ) : null}
      {resetDialog}
    </div>
  );

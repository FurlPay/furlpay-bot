...
import type { ActionApprovalRule, ActionAutoReviewSettings } from "@furlpay-bot/contracts";
...
export function ApprovalRulesSettings() {
  ...
  return (
    <div data-testid="action-confirmation-settings" className="pt-5">
      ...
      <p className="mt-2 text-[13.5px] leading-[1.5] text-[var(--fp-muted)]">
        <Trans>
      ...
      <div className="mt-4 flex flex-col items-start gap-2">
        <button
          ...
          className="rounded-[11px] border border-[var(--fp-border)] px-[17px] py-2 text-[14px] text-[var(--fp-soft)] disabled:opacity-50"
        >
      ...
      <label className="mt-5 flex cursor-pointer items-start gap-3">
        ...
        />
        <span>
          <span className="block text-[14px] text-[var(--fp-soft)]">
            <Trans>Flag unexpected actions</Trans>
          </span>
          {autoReview?.enabled && !autoReview.checkerAvailable ? (
            <span className="mt-1 block text-[13px] text-[var(--fp-muted)]">
      ...
      {loading ? (
        <p className="mt-4 text-[13px] text-[var(--fp-muted)]">
        ...
        <ul className="mt-4 space-y-2">
          {rules.map((rule) => (
            <li
              key={rule.id}
              className="flex items-center justify-between gap-3 rounded-[11px] border border-[var(--fp-border)] px-3.5 py-2.5"
            >
              <span className="text-[13.5px] text-[var(--fp-soft)]">{describeRule(rule)}</span>
              <button
                type="button"
                onClick={() => void removeRule(rule.id)}
                className="text-[13px] text-[var(--fp-muted)]"
              >
                <Trans>Remove</Trans>
              </button>
            </li>

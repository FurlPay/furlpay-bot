...
import type { ServerUpdateCheck, ServerUpdateStatus } from "@furlpay-bot/contracts";
...
export function SoftwareUpdatePanel({
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      {error ? (
        <p role="alert" className="text-[12.5px] text-[var(--fp-danger)]">
          {error}
        </p>
      ) : null}
...
function CheckSummary({ check }: { check: ServerUpdateCheck }) {
  if (check.status === "up-to-date") {
    return (
      <p className="text-[12.5px] text-[var(--fp-muted-2)]">
  ...
  if (check.status === "available") {
    return (
      <p className="text-[12.5px] text-[var(--fp-soft)]">
  ...
  if (check.status === "dirty") {
    return (
      <p className="text-[12.5px] text-[var(--fp-danger)]">

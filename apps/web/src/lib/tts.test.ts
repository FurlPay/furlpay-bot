...
function stubSelectedSpace(id: string) {
  const store = new Map<string, string>([["furlpay-bot:space-id", id]]);
  const localStorage = {
  ...
  return (next: string) => store.set("furlpay-bot:space-id", next);
}
...
describe("Speaker", () => {
  ...
  it("keeps speak requests in the space where playback started", async () => {
    ...
    expect(headers.get("x-furlpay-bot-space-id")).toBe("space-support");

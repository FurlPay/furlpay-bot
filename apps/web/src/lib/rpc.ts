...
import type { AppContract } from "@furlpay-bot/contracts";

const SPACE_STORAGE_KEY = "furlpay-bot:space-id";
...
/** Adds `x-furlpay-bot-space-id` when a space is selected. */
export function withSpaceHeaders(
  init?: HeadersInit,
  spaceId: string | null = selectedSpaceId(),
): Headers {
  const headers = new Headers(init);
  if (spaceId) headers.set("x-furlpay-bot-space-id", spaceId);
  else headers.delete("x-furlpay-bot-space-id");
  return headers;
}

...
import {
  ...
  type FurlPayBotDesktop,
  type FurlPayBotDesktopOAuthCallback,
  windowChromeKind,
} from "./desktop.js";

function desktop(platform: string): FurlPayBotDesktop {
  const updateState = {
...
describe("window chrome", () => {
  ...
  it("keeps conversation header controls clickable", () => {
    ...
    expect(shell).toContain(
      'className="app-drag flex items-center justify-between border-b border-[var(--fp-surface)]',
    );
...
describe("attempt correlation", () => {
  ...
  function bridgeEmitting() {
    let emit: (callback: FurlPayBotDesktopOAuthCallback) => void = () => undefined;
    const unsubscribe = vi.fn();
    vi.stubGlobal("window", {
      furlpayBotDesktop: {
        ...desktop("linux"),
        oauth: {
          onCallback: (listener: (callback: FurlPayBotDesktopOAuthCallback) => void) => {
            emit = listener;
            return unsubscribe;
          },
        },
    ...
    return { emit: (c: FurlPayBotDesktopOAuthCallback) => emit(c), unsubscribe };
  }

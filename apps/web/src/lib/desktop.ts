import type { FurlPayBotDesktop, FurlPayBotDesktopOAuthCallback } from "@furlpay-bot/contracts";

export type { FurlPayBotDesktop, FurlPayBotDesktopOAuthCallback } from "@furlpay-bot/contracts";

declare global {
  interface Window {
    furlpayBotDesktop?: FurlPayBotDesktop;
  }
}

export function desktopBridge(): FurlPayBotDesktop | undefined {
  return typeof window === "undefined" ? undefined : window.furlpayBotDesktop;
}

/** The compact `code#state` form the manual paste flow already accepts. */
export function desktopOAuthCode(callback: FurlPayBotDesktopOAuthCallback) {
...
export function windowChromeKind(desktop?: FurlPayBotDesktop): "spacer" | "darwin" | "controls" {

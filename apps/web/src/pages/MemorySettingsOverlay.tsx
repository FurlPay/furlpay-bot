...
import type { SpaceMemoryConfig } from "@furlpay-bot/contracts";
...
function ScopePicker({
  ...
  return (
    <div className="text-[13.5px] text-[var(--fp-muted)]">
...
export function MemorySettingsOverlay({
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-4 sm:p-10">
      <div className="flex max-h-[min(760px,100%)] w-[560px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[var(--fp-hairline-strong)] bg-[var(--fp-surface)] shadow-[0_40px_90px_rgba(0,0,0,.55)]">
        <div className="flex items-start justify-between px-6 pt-6 sm:px-8 sm:pt-7">
          ...
          <button
            ...
            disabled={busy}
            onClick={onClose}
            className="text-[var(--fp-muted)] disabled:opacity-40"
          >
            ✕
          </button>
        </div>
          ...
          {config === undefined ? (
            ...
            <>
              {MEMORY_PROVIDER_SETTINGS.length > 1 ? (
                <label className="mb-4 block text-[13.5px] text-[var(--fp-muted)]">
                  <Trans>Provider</Trans>
                  <select
                    value={selectedProvider}
                    disabled={busy}
                    onChange={(event) => setSelectedProvider(event.target.value)}
                    className="mt-2 w-full rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3.5 py-3 text-[var(--fp-ink)] outline-none disabled:opacity-40"
                  >
            ...
            <p className="text-sm text-[var(--fp-danger)]">

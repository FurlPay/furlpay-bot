...
import type { CapabilityInstall, ConnectionCatalogItem } from "@furlpay-bot/contracts";
import {
  abortableDelay,
...
export function PluginsOverlay({
  ...
  async function connect(item: ConnectionCatalogItem) {
    ...
    try {
      ...
      if (started.authorizationUrl)
        window.open(started.authorizationUrl, "furlpay-bot-plugin-connect", "noopener,noreferrer");
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-10">
      <div className="flex h-[760px] w-[1080px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[var(--fp-hairline-strong)] bg-[var(--fp-surface)] shadow-[0_40px_90px_rgba(0,0,0,.55)]">
        ...
        <div className="px-8 pt-4">
          <input
            ...
            className="w-full rounded-[13px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-4 py-3 text-[15px] text-[var(--fp-ink)] outline-none"
          />
        </div>

        <div id="integration-list" className="rk-scroll flex-1 overflow-y-auto px-8 py-6">
          {catalogError ? (
            <p className="mb-4 text-sm text-[var(--fp-danger)]">{catalogError}</p>
          ) : null}
          {loading ? (
            <p className="text-[var(--fp-muted-2)]">
          ...
          {showFeatured ? (
            <div className="mb-6" data-testid="featured-connectors">
              {!loading && catalog.length === 0 ? (
                <p className="text-[13.5px] leading-6 text-[var(--fp-muted-2)]">
                  {EMPTY_PLUGIN_CATALOG_MESSAGE}
                </p>
              ) : (
                      ...
                      >
                        {item?.logo ? (
                          <img
                            src={item.logo}
                            alt=""
                            className="h-9 w-9 shrink-0 rounded-xl bg-[var(--fp-scroll)] object-contain"
                          />
                        ) : (
                          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--fp-scroll)] text-sm font-semibold text-[var(--fp-ink)]">
                            {tile.label[0]}
                          </div>
                        )}
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-[15px] font-medium text-[var(--fp-ink)]">
                            {tile.label}
                          </div>
                          {disabled ? (
            ...
            <div className="mt-4 space-y-4">
              {onOpenMcp ? (
                <button
                  type="button"
                  onClick={onOpenMcp}
                  className="rounded-full border border-[var(--fp-border)] px-3 py-1.5 text-xs text-[var(--fp-soft)] hover:bg-[var(--fp-hairline-strong)]"
                >
              ...
              {sourceError ? (
                <p className="text-sm text-[var(--fp-danger)]">{sourceError}</p>
              ) : null}

              {sourceKind ? (
                <div className="space-y-3 rounded-[16px] border border-[var(--fp-scroll)] bg-[var(--fp-inset)] p-5">
                  <div className="text-base font-medium text-[var(--fp-ink)]">
                    {sourceKind === "treg" ? (
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-[var(--fp-scroll)] bg-[var(--fp-hairline)] px-3 py-2.5 text-sm text-[var(--fp-ink)] outline-none"
                  />
                  {sourceKind !== "treg" ? (
                    <input
                      value={sourceUrl}
                  ...
                  <p className="text-xs leading-5 text-[#707077]">
                    <Trans>
                      FurlPay Bot verifies the source before saving it. Credentials are encrypted and are
              ...
              <div>
                <div className="mb-3 text-sm font-medium text-[var(--fp-soft)]">
                ...
                {sources.map((source) => (
                  ...
                  >
                    <div className="grid h-[42px] w-[42px] place-items-center rounded-xl bg-[var(--fp-scroll)] font-semibold uppercase text-[var(--fp-ink)]">
                      {source.kind === "mcp" ? "M" : "A"}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[15.5px] font-medium text-[var(--fp-ink)]">
                        {source.name}
                      </div>
                      <div className="truncate text-[13.5px] text-[var(--fp-faint)]">

...
import { type Bot, GROUP_MEMBER_MAX, GROUP_MEMBER_MIN, type Group } from "@furlpay-bot/contracts";
import { BotAvatar, Button } from "@furlpay-bot/ui-web";
        ...
        return (
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl px-2.5 py-2.5 text-start ${
              checked ? "bg-[var(--fp-surface-2)]" : "hover:bg-[var(--fp-surface)]"
            }`}
          >
            <BotAvatar color={bot.color} identity={bot.id} size={32} status={bot.status} />
            <span className="flex-1 text-[15px] text-[var(--fp-ink)]" dir="auto">
              {bot.name}
            </span>
            <span className="text-[13px] text-[var(--fp-muted-2)]">{checked ? "✓" : ""}</span>
          </button>
        );
      })}
    </div>
...
export function CreateGroupForm({
  ...
  return (
    <div>
      ...
      {error ? (
        <p role="alert" className="mb-3 text-[13px] text-[var(--fp-danger)]">
          {error}
        </p>
      ) : null}

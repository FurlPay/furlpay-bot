  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-4 sm:p-10">
      <div
        ...
        tabIndex={-1}
        className="rk-scroll max-h-full w-[640px] max-w-full overflow-y-auto rounded-[26px] border border-[var(--fp-hairline-strong)] bg-[var(--fp-surface)] p-6 shadow-[0_40px_90px_rgba(0,0,0,.55)] sm:p-8"
      >
        ...
        <section className="mt-8 rounded-[14px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-4 py-4">
          ...
          {status?.identities.length ? (
            <ul className="mt-3 space-y-3">
              {status.identities.map((identity) => (
                <li
                  key={identity.id}
                  className="flex items-center justify-between gap-3 text-[14px] text-[var(--fp-soft)]"
                >
                  <span>
                    {providerLabel(identity.provider)} · {identity.address}{" "}
                    <span className="text-[12px] text-[var(--fp-faint)]">→ {identity.botName}</span>
                  </span>
                  <BuiButton
                    onClick={() =>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <select
              ...
              className="rounded-[10px] border border-[var(--fp-scroll)] bg-[var(--fp-surface)] px-3 py-2 text-[13.5px] text-[var(--fp-ink)]"
            >
          ...
          {linkCode ? (
            <p className="mt-3 text-[14px] text-[var(--fp-soft)]" data-testid="messaging-link-code">
              <Trans>
                Send <span className="font-mono text-[var(--fp-ink-strong)]">{linkCode}</span> to
              ...
              {channels.map((channel) => (
                ...
                >
                  <span>
                    ...
                    <span className="text-[12px] text-[var(--fp-faint)]">
                      {channelMeta(channel)}
                    </span>
                  </span>
              ...
              {connections.map((connection) => (
                ...
                >
                  <span>
                    ...
                    <span className="text-[12px] text-[var(--fp-faint)]">{connection.status}</span>
                  </span>
                  <span className="flex gap-2">

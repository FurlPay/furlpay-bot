...
export function ActivityList({ onOpenRun }: ActivityListProps) {
  ...
  if (loading) {
    return (
      <div className="px-2.5 py-2 text-[13px] text-[var(--fp-muted-2)]">
  ...
  return (
    <div className="mb-2 border-b border-[var(--fp-hairline-strong)] pb-2">
      {activeRuns.length > 0 ? (
        <section>
          <div className="px-2.5 pb-1 pt-1 text-[12.5px] font-medium text-[var(--fp-muted-2)]">
            <Trans>Now</Trans>
          </div>
          {activeRuns.map((run) => (
...
function ActivityRow({ run, onOpen }: { run: RunActivityRow; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onOpen}
      className="flex w-full gap-3 rounded-xl px-2.5 py-[9px] text-left hover:bg-[var(--fp-page)]"
    >
      <span
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="truncate text-[14px] font-medium text-[var(--fp-ink)]">{title}</span>
          <span className="shrink-0 text-[12px] text-[var(--fp-muted-2)]">
            {formatRelativeTime(run.updatedAt)}
          </span>
        </div>
        <div className="mt-0.5 flex items-baseline gap-2">
          {run.promptSnippet ? (
            <span className="min-w-0 flex-1 truncate text-[13px] text-[var(--fp-muted)]">
              {run.promptSnippet}
            </span>
          ) : null}

...
import type { ScratchpadItem } from "@furlpay-bot/contracts";
...
export function ScratchpadSection({ botId }: { botId: string }) {
  ...
  return (
    <div className="mt-6" data-testid="bot-scratchpad">
      <div className="mb-3 text-[14px] text-[var(--fp-muted)]">
        <Trans>Open work</Trans>
      </div>
      {items.length === 0 ? (
        <div className="px-2.5 py-1 text-[13.5px] text-[var(--fp-muted-2)]">
        ...
        items.map((item) => (
          <div
            key={item.id}
            className="flex w-full items-start gap-2 rounded-[11px] px-2.5 py-2.5 hover:bg-[var(--fp-inset)]"
          >
            <button
              type="button"
              ...
              className="mt-0.5 h-4 w-4 shrink-0 rounded border border-[var(--fp-border)] text-[10px] leading-none text-[#E65707]"
            >
              {item.status === "done" ? "✓" : ""}
            </button>
            <div className="min-w-0 flex-1">
              <div
                className={`text-start text-[14.5px] ${item.status === "done" ? "text-[var(--fp-muted-2)] line-through" : "text-[var(--fp-ink)]"}`}
                dir="auto"
              >
                {item.title}
              </div>
              {item.notes ? (
                <div className="mt-0.5 text-[12.5px] text-[var(--fp-muted-2)]" dir="auto">
                  {item.notes}
                </div>
              ) : null}
            </div>
            <span className="shrink-0 text-[12px] text-[var(--fp-muted-2)]">{item.status}</span>
            {item.status === "open" ? (
              <button
                type="button"
                aria-label={t`Park`}
                ...
                className="shrink-0 text-[12px] text-[var(--fp-faint)]"
              >
                <Trans>Park</Trans>
              </button>
            ) : item.status === "parked" ? (
      ...
      >
        <input
          ...
          maxLength={200}
          className="min-w-0 flex-1 rounded-[11px] border border-[var(--fp-border)] bg-transparent px-3 py-2 text-[14px] text-[var(--fp-ink)] placeholder:text-[#55555A]"
        />

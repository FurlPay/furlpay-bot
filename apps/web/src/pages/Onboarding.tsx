...
export function OnboardingPage() {
  ...
  return (
    <div className="flex min-h-full items-center justify-center bg-[var(--fp-main)] px-6">
      <div className="w-[560px]">
        ...
        {step === "model" ? (
          <div>
            <h1 className="text-[32px] font-medium text-[var(--fp-ink-strong)]">
              <Trans>Connect a model</Trans>
            </h1>
            <p className="mt-2 text-[var(--fp-muted)]">
            ...
            <input
              ...
              className="mt-8 w-full rounded-[11px] border border-[var(--fp-border)] bg-transparent px-3.5 py-3 text-[var(--fp-ink)]"
            />
            <div className="mt-3 max-h-48 overflow-y-auto rounded-[11px] border border-[var(--fp-border)]">
              {filteredProviders.map((entry) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between border-b border-[var(--fp-hairline-strong)] px-3.5 py-2.5 text-left last:border-0 ${
                    entry.provider === provider
                      ? "bg-[var(--fp-surface-2)]"
                      : "hover:bg-[var(--fp-inset)]"
                  }`}
                >
                  <span className="text-[15px] text-[var(--fp-ink)]">
                    {entry.providerName ?? entry.provider}
                  </span>
                  <span className="text-[12px] text-[var(--fp-muted)]">
                    {localizedProviderHint(entry)}
                  </span>
                </button>
              ))}
            </div>
            <div className="mt-4 block text-sm text-[var(--fp-muted)]">
              {isOpenAiCompatible ? (
                <>
                  ...
                  <div className="mt-3">
                    <button
                      ...
                      className="rounded-[11px] border border-[var(--fp-border)] px-4 py-2 text-sm text-[var(--fp-ink)] disabled:opacity-40"
                    >
                    ...
                    {probeModels.length && probeModels.includes(modelId) ? (
                      <div className="relative mt-2">
                        <select
                          ...
                          className="w-full appearance-none rounded-[11px] border border-[var(--fp-border)] bg-transparent py-3 pl-3.5 pr-11 text-[var(--fp-ink)]"
                        >
                          {probeModels.map((id) => (
            ...
            {!isOpenAiCompatible && selected?.billing ? (
              <p className="mt-2 text-[13px] text-[var(--fp-muted)]">{selected.billing}</p>
            ) : null}
            {subscriptionSignIn ? (
              <div className="mt-4">
                {oauth ? (
                  <div className="rounded-[11px] border border-[var(--fp-border)] px-3.5 py-3">
                    {oauth.mode === "auth-url" ? (
                      <>
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="rounded-[11px] bg-[var(--fp-cream)] px-4 py-2.5 text-[var(--fp-cream-ink)] disabled:opacity-40"
                          >
                            <Trans>Submit</Trans>
                          </button>
                        </div>
                  ...
                  <button
                    ...
                    className="rounded-[11px] bg-[var(--fp-cream)] px-5 py-2.5 text-[var(--fp-cream-ink)] disabled:opacity-40"
                  >
            ...
            {acceptsKey ? (
              isOpenAiCompatible ? (
                <details className="mt-4 text-sm text-[var(--fp-muted)]">
                  <summary className="w-fit cursor-pointer select-none">
                    <Trans>API key</Trans>
                  </summary>
                  <input
                ...
                <label className="mt-4 block text-sm text-[var(--fp-muted)]">
            ...
            {notice ? <p className="mt-3 text-sm text-[var(--fp-success-soft)]">{notice}</p> : null}
            {error ? <p className="mt-3 text-sm text-[var(--fp-danger)]">{error}</p> : null}
        ...
        {step === "bot" ? (
          <div>
            ...
            <label className="mt-8 block text-sm text-[var(--fp-muted)]">
              <Trans>Name</Trans>
              <input
                value={name}
            ...
            <button
              ...
              className="mt-6 rounded-[11px] bg-[var(--fp-cream)] px-5 py-2.5 text-[var(--fp-cream-ink)] disabled:opacity-40"
            >
              <Trans>Continue</Trans>
            </button>
          </div>

...
import type { Me } from "@furlpay-bot/contracts";
import {
  OPENAI_COMPATIBLE_PROVIDER_ID,
  openAiCompatibleConnectReady,
  openAiCompatibleProbeSuccessMessage,
} from "@furlpay-bot/contracts";
import { Button } from "@furlpay-bot/ui-web";
...
export function ModelSettingsOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-4 sm:p-10">
      <div className="flex h-[min(760px,100%)] w-[1080px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[var(--fp-hairline-strong)] bg-[var(--fp-surface)] shadow-[0_40px_90px_rgba(0,0,0,.55)]">
        <div className="flex items-start justify-between px-6 pt-6 sm:px-8 sm:pt-7">
          <div>
            <div className="text-2xl font-medium text-[var(--fp-ink-strong)]">
              <Trans>Models</Trans>
            </div>
            <p className="mt-1 text-[13.5px] text-[var(--fp-faint)]">
              {loading ? (
                <Trans>Loading model catalog…</Trans>
              ) : (
                <Trans>Choose which connected model FurlPay Bot uses.</Trans>
              )}
            </p>
          </div>
          <button
            ...
            onClick={handleClose}
            className="text-[var(--fp-muted)]"
          >
            ✕
          </button>
        </div>

        <div className="mx-6 mt-5 rounded-[14px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-4 py-3 sm:mx-8">
          <div className="text-[12.5px] uppercase tracking-[0.08em] text-[var(--fp-muted-2)]">
            <Trans>Active model</Trans>
          </div>
          <div className="mt-1 text-[16px] text-[var(--fp-ink-strong)]">
            {currentEntry?.label ?? me?.defaultModel ?? t`Deployment default`}
          </div>
          <div className="mt-1 text-[13px] text-[var(--fp-muted)]">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-hidden px-6 py-6 sm:px-8 md:flex-row">
          <div className="flex min-h-0 shrink-0 flex-col md:w-[310px]">
            <div className="mb-3 text-[13.5px] text-[var(--fp-muted)]">
            ...
            <input
              ...
              className="w-full rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3.5 py-2.5 text-[14px] text-[var(--fp-ink)] outline-none placeholder:text-[var(--fp-muted-2)] focus:border-[var(--fp-muted-2)]"
            />
            <div className="rk-scroll mt-3 max-h-[240px] overflow-y-auto rounded-[13px] border border-[var(--fp-border)] md:min-h-0 md:max-h-none md:flex-1">
              {filteredGroups.length ? (
                filteredGroups.map((group) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 border-b border-[var(--fp-hairline-strong)] px-3.5 py-3 text-start last:border-0 ${
                        group.id === provider
                          ? "bg-[var(--fp-surface-2)]"
                          : "hover:bg-[var(--fp-inset)]"
                      }`}
                    >
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[15px] text-[var(--fp-ink)]">
                          {group.name}
                        </span>
                        <span className="mt-0.5 block text-[12px] text-[var(--fp-muted-2)]">
                      ...
                      {connected ? (
                        <span className="text-[12px] text-[var(--fp-success-soft)]">
                ...
                <p className="px-3.5 py-4 text-[13px] text-[var(--fp-muted)]">
          ...
          <div ref={detailScrollRef} className="rk-scroll min-h-0 min-w-0 flex-1 overflow-y-auto">
            {error ? <p className="mb-4 text-sm text-[var(--fp-danger)]">{error}</p> : null}
            {notice ? <p className="mb-4 text-sm text-[var(--fp-success-soft)]">{notice}</p> : null}
            {selected ? (
              <>
                <div className="block text-[13.5px] text-[var(--fp-muted)]">
                  {isOpenAiCompatible ? (
                    <>
                      <label className="block">
                        ...
                        <input
                          ...
                          autoComplete="off"
                          className="mt-2 w-full rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3.5 py-3 text-[var(--fp-ink)] outline-none"
                        />
                      </label>
                      <details className="mt-2 text-[13px] leading-[1.5] text-[var(--fp-muted)]">
                        <summary className="w-fit cursor-pointer select-none">
                          <Trans>Setup help</Trans>
                        </summary>
                        <p className="mt-1">
                          {t`Paste the OpenAI-compatible address from your server. FurlPay Bot adds /v1 if needed.`}
                        </p>
                      </details>
                      ...
                      <div className="mt-4 block">
                        ...
                        {probeModels.length && probeModels.includes(modelId) ? (
                          <div className="relative mt-2">
                            <select
                              ...
                              className="w-full appearance-none rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] py-3 pl-3.5 pr-11 text-sm text-[var(--fp-ink)]"
                            >
                              {probeModels.map((id) => (
                            ...
                            <span
                              aria-hidden="true"
                              className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[var(--fp-muted)]"
                            >
                        ...
                        {probeModels.length && !probeModels.includes(modelId) ? (
                          <button
                            type="button"
                            className="mt-2 text-[13px] text-[var(--fp-muted)] underline"
                ...
                {!isOpenAiCompatible && selected.billing ? (
                  <p className="mt-2 text-[13px] leading-[1.5] text-[var(--fp-muted)]">
                    {selected.billing}
                  </p>
                ) : null}

                {!isOpenAiCompatible ? (
                  <div className="mt-5 rounded-[13px] border border-[var(--fp-border)] px-4 py-3">
                    <div className="text-[12.5px] uppercase tracking-[0.08em] text-[var(--fp-muted-2)]">
                      <Trans>Personal credential</Trans>
                    </div>
                    <div className="mt-1 text-[15px] text-[var(--fp-ink)]">
                      {credential ? (
                ...
                {subscriptionSignIn ? (
                  <div className="mt-5">
                    {oauth ? (
                      <div className="rounded-[13px] border border-[var(--fp-border)] px-4 py-3">
                        {oauth.mode === "auth-url" ? (
                          <>
                            <p className="text-sm leading-[1.5] text-[var(--fp-muted)]">
                              <Trans>
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-[var(--fp-ink)] underline"
                                >
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              <input
                                ...
                                className="w-full rounded-[11px] border border-[var(--fp-border)] bg-transparent px-3.5 py-2.5 text-[13px] text-[var(--fp-ink)]"
                              />
                              <Button
                                type="button"
                                variant="outline"
                          ...
                          <>
                            ...
                            <p className="mt-2 font-mono text-[22px] tracking-[0.2em] text-[var(--fp-ink-strong)]">
                              {oauth.userCode}
                            </p>
                            <p className="mt-2 text-sm text-[var(--fp-muted)]">
                              <Trans>Waiting for sign-in…</Trans>
                ...
                {acceptsKey ? (
                  <div className="mt-5">
                    {isOpenAiCompatible ? (
                      <details className="text-[13.5px] text-[var(--fp-muted)]">
                        <summary className="w-fit cursor-pointer select-none">
                          <Trans>API key</Trans>
                        </summary>
                        <input
                      ...
                      <label className="block text-[13.5px] text-[var(--fp-muted)]">
                        {credential ? (
                ...
                {selected.auth === "oauth" && !subscriptionSignIn ? (
                  <p className="mt-5 text-sm leading-[1.5] text-[var(--fp-muted)]">
                    <Trans>
                      This subscription sign-in is not available in FurlPay Bot yet. Use a deployment
  ...
  return (
    <div ref={rootRef} className="relative mt-2">
      <button
        ...
        className="flex w-full items-center justify-between rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3.5 py-3 text-start text-[var(--fp-ink)] outline-none focus-visible:border-[var(--fp-muted-2)]"
        onClick={() => setOpen((current) => !current)}
        onKeyDown={onTriggerKeyDown}
      >
        <span className="min-w-0 truncate">{options[selectedIndex]?.label}</span>
        <span className="ml-3 shrink-0 text-[var(--fp-muted)]" aria-hidden="true">
          <ChevronDown size={16} strokeWidth={1.8} />
        </span>
      </button>
      {open ? (
        <div className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-[11px] border border-[var(--fp-border)] bg-[var(--fp-inset)] shadow-[0_20px_45px_rgba(0,0,0,.55)]">
          <input
            ref={searchRef}
            type="text"
            value={query}
            ...
            className="w-full border-b border-[var(--fp-border)] bg-transparent px-3 py-2.5 text-[13.5px] text-[var(--fp-ink)] outline-none placeholder:text-[var(--fp-muted-2)]"
          />
          <div
            id={listboxId}
            role="listbox"
          ...
          >
            {groupRanges.map((group) => (
              <div key={group.name}>
                <p className="px-3 pb-1 pt-2 text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--fp-muted-2)]">
                  {group.name}
                </p>
            ...
            {filteredOptions.length === 0 ? (
              <p className="px-3 py-2 text-[13px] text-[var(--fp-muted)]">
...
function ModelOption({
  ...
  return (
    <button
      ...
      className={`flex w-full items-center justify-between gap-3 px-3 py-2 text-start text-[13.5px] text-[var(--fp-ink)] outline-none hover:bg-[var(--fp-surface-2)] focus-visible:bg-[var(--fp-surface-2)] ${
        highlighted || option.id === value ? "bg-[var(--fp-surface-2)]" : ""
      }`}
    ...
    >
      ...
      {option.billing.toLowerCase().includes("free") ? (
        <span className="shrink-0 text-[12px] text-[var(--fp-muted)]">{t`Free`}</span>
      ) : null}
    </button>
  );
}

...
export function AuthPage({ mode }: { mode: AuthMode }) {
  ...
  const title =
    mode === "in" ? (
      <Trans>Sign in to FurlPay Bot</Trans>
    ) : mode === "up" ? (
      <Trans>Create your FurlPay Bot</Trans>
    ) : (
  ...
  return (
    <div className="flex min-h-full items-center justify-center bg-[var(--fp-page)] px-6 py-16 text-[var(--fp-ink)]">
      <form onSubmit={submit} className="flex w-[460px] flex-col items-center">
        <div className="flex h-[74px] w-[74px] items-center justify-center gap-[11px] rounded-full bg-[var(--fp-surface)]">
          <span className="h-5 w-[9px] rounded-full bg-[var(--fp-cream)]" />
          <span className="h-5 w-[9px] rounded-full bg-[var(--fp-cream)]" />
        </div>
        <h1 className="mb-[38px] mt-[30px] text-[38px] tracking-[-0.02em]">{title}</h1>
        {sent ? (
          <div role="status" className="w-full text-center">
            <p className="text-[17px] text-[var(--fp-ink)]">
            ...
            <Link to="/sign-in" className="mt-6 inline-block font-medium text-[var(--fp-ink)]">
          ...
          <>
              ...
              <div className="mt-4 w-full text-[16px] text-[#6E6E68]">
                ...
                <div className="relative mt-2">
                  ...
                  <button
                    ...
                    className="absolute inset-y-0 right-0 flex items-center px-[18px] text-[#8C8C86] hover:text-[var(--fp-ink)]"
                  >
                    {showPassword ? (
                      <svg
                        width="20"
                ...
                {mode === "in" && reset?.passwordReset ? (
                  <div className="mt-2 text-right text-[14px]">
                    <Link to="/forgot-password" className="font-medium text-[var(--fp-ink)]">
            ...
            <button
              type="submit"
              disabled={pending}
              className="mt-3 w-full rounded-[13px] bg-[var(--fp-elevated)] py-[18px] text-center text-[17px] font-medium text-[var(--fp-cream)] hover:bg-[var(--fp-scroll)]"
            >
              {pending ? (
                <Trans>Working…</Trans>
              ) : mode === "in" ? (
            ...
            <p className="mt-[30px] text-[16px] text-[#8C8C86]">
              {mode === "in" ? (
                <>
                  ...
                  <Link to="/sign-up" className="font-medium text-[var(--fp-ink)]">
                    <Trans>Sign up</Trans>
                  </Link>
                </>
              ) : mode === "up" ? (
                <>
                  ...
                  <Link to="/sign-in" className="font-medium text-[var(--fp-ink)]">
                    <Trans>Sign in</Trans>
                  </Link>
                </>
              ) : (
        ...
        {complete ? (
          ...
          <>
            ...
            <button
              ...
              className="mt-4 w-full rounded-[13px] bg-[var(--fp-elevated)] py-[18px] text-[17px] font-medium text-[var(--fp-cream)] disabled:opacity-60"
            >

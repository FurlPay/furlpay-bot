...
import type { Bot, BotMcpServer, McpServer, McpTransport } from "@furlpay-bot/contracts";
import { deriveMcpSlug } from "@furlpay-bot/core";
...
export function McpServersOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-6">
      <section
        className="flex max-h-full w-[1080px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[var(--fp-border)] bg-[var(--fp-surface)] shadow-[0_40px_90px_rgba(0,0,0,.55)]"
        aria-label={t`MCP servers`}
      >
        <header className="flex items-start justify-between border-b border-[var(--fp-hairline-strong)] px-8 py-6">
          <div>
            <h1 className="text-2xl font-medium text-[var(--fp-ink-strong)]">
          ...
          <button
            ...
            onClick={onClose}
            className="text-xl text-[var(--fp-muted)]"
          >
            ✕
          </button>
        </header>
        ...
        <div className="rk-scroll grid min-h-0 grid-cols-1 gap-6 overflow-y-auto p-8 lg:grid-cols-[1fr_1.08fr]">
          <div className="rounded-2xl border border-[var(--fp-border)] bg-[var(--fp-inset)] p-5">
            <h2 className="text-[15px] font-medium text-[var(--fp-ink)]">
            ...
            <label className="mb-1.5 block text-xs text-[var(--fp-soft)]" htmlFor="mcp-name">
            ...
            <input
              ...
              className="mb-4 w-full rounded-xl border border-[var(--fp-border)] bg-[var(--fp-sidebar)] px-3 py-2.5 text-sm text-[var(--fp-ink)] outline-none"
            />
            <div className="mb-4 grid grid-cols-3 gap-1 rounded-xl border border-[var(--fp-border)] bg-[var(--fp-sidebar)] p-1">
              {(
                [
            ...
            {transport === "stdio" ? (
              <>
                <label className="mb-1.5 block text-xs text-[var(--fp-soft)]" htmlFor="mcp-command">
                  <Trans>Command</Trans>
                </label>
                <input
                  id="mcp-command"
                  ...
                  className="mb-4 w-full rounded-xl border border-[var(--fp-border)] bg-[var(--fp-sidebar)] px-3 py-2.5 text-sm text-[var(--fp-ink)] outline-none"
                />
                <label className="mb-1.5 block text-xs text-[var(--fp-soft)]" htmlFor="mcp-args">
              ...
              <>
                <label
                  className="mb-1.5 block text-xs text-[var(--fp-soft)]"
            ...
            <label className="mb-1.5 block text-xs text-[var(--fp-soft)]" htmlFor="mcp-secret">
            ...
            <input
              ...
              className="mb-3 w-full rounded-xl border border-[var(--fp-border)] bg-[var(--fp-sidebar)] px-3 py-2.5 text-sm text-[var(--fp-ink)] outline-none"
            />
            {transport !== "stdio" ? (
              <div className="grid grid-cols-[.7fr_1fr] gap-2">
                <input
                  ...
                  className="rounded-xl border border-[var(--fp-border)] bg-[var(--fp-sidebar)] px-3 py-2.5 text-xs text-[var(--fp-ink)] outline-none"
                />
                <input
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="mt-3 space-y-2">
                {bots.map((bot) => (
                  <label
                    key={bot.id}
                    className="flex cursor-pointer items-center gap-3 rounded-xl border border-[var(--fp-border)] bg-[var(--fp-inset)] px-3 py-3"
                  >
                    <input
                      type="checkbox"
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                {servers.length === 0 ? (
                  <p className="rounded-xl border border-dashed border-[var(--fp-elevated)] p-5 text-sm text-[#77777F]">
                  ...
                  servers.map((server) => (
                    <div
                      key={server.id}
                      className="rounded-xl border border-[var(--fp-border)] bg-[var(--fp-inset)] p-4"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-[var(--fp-ink)]">{server.name}</span>
                        ...
                        {bots.map((bot) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-full border px-2.5 py-1 text-[11px] ${assigned ? "border-[#7785FF] bg-[#30356A] text-[#E2E4FF]" : "border-[var(--fp-elevated)] text-[#85858B]"}`}
                            >
                              {assigned ? "✓ " : ""}
                              {bot.name}
                            </button>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          <>
                            ...
                            {server.oauthStatus !== "none" ? (
                              <button
                                ...
                                className="rounded-lg border border-[var(--fp-elevated)] px-3 py-2 text-xs text-[var(--fp-soft)]"
                              >
                        ...
                        <button
                          ...
                          className={`ml-auto rounded-lg border px-3 py-2 text-xs ${confirmingDelete === server.id ? "border-[#B4434F] bg-[#3A1A20] text-[#F3A2AA]" : "border-[var(--fp-elevated)] text-[var(--fp-soft)]"}`}
                        >

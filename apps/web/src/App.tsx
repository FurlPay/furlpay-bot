...
export function App() {
  ...
  if (gate === "loading") {
    return window.location.pathname.startsWith("/app") ? (
      ...
      <div
        className="grid h-full place-items-center text-[var(--fp-muted-2)]"
        data-furlpay-bot-app-state="session-pending"
      >
        <Trans>Loading…</Trans>
      </div>
    );
  ...
  return (
    <div className="h-full" data-furlpay-bot-app-state="ready">
      <Suspense fallback={<div className="h-full bg-[var(--fp-page)]" />}>
        <Routes>
...
function SessionUnavailable({ refetch }: { refetch: () => Promise<void> }) {
  ...
  return (
    <div className="grid h-full place-items-center bg-[var(--fp-page)] px-6 text-center">
      <div className="flex flex-col items-center">
        <LoadingState label={t`Reconnecting`} />
        <p className="mt-3 text-[13.5px] text-[var(--fp-muted-2)]">
...
function ShellSkeleton() {
  return (
    <div
      className="flex h-full overflow-hidden bg-[var(--fp-page)]"
      data-furlpay-bot-app-state="session-pending"
    >
      <aside className="hidden w-[316px] shrink-0 border-e border-[var(--fp-hairline)] bg-[var(--fp-sidebar)] px-3.5 pt-16 md:block">
        <div className="h-10 rounded-xl bg-[var(--fp-surface)]" />
        <div className="mt-5 space-y-2 px-1">
          {[0, 1, 2, 3].map((row) => (
            <div key={row} className="flex items-center gap-3 rounded-xl px-2 py-2.5">
              <div className="h-9 w-9 rounded-full bg-[var(--fp-elevated)]" />
              <div className="flex-1 space-y-2">
                <div className="h-3 w-2/5 rounded bg-[var(--fp-surface-2)]" />
                <div className="h-2.5 w-4/5 rounded bg-[var(--fp-inset)]" />
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="flex flex-1 flex-col">
        <div className="h-[74px] border-b border-[var(--fp-surface)]" />
        <div className="flex flex-1 items-center justify-center text-[14px] text-[var(--fp-muted)]">
          <Trans>Opening your Space…</Trans>
        </div>
        <div className="mx-6 mb-6 h-[54px] rounded-full border border-[var(--fp-hairline-strong)] bg-[var(--fp-page)]" />
      </main>
    </div>
  );
}

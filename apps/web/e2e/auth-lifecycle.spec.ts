...
test("logout protects bot deep links and sign-in restores the session", async ({
  ...
  const email = `auth-lifecycle-${stamp}@furlpay-bot.test`;
  ...
  await expect(page.getByRole("heading", { name: "Sign in to FurlPay Bot" })).toBeVisible();
  await page.goto("/");
...
test("changes and recovers an email password", async ({ page }, testInfo) => {
  const stamp = Date.now();
  const email = `password-recovery-${stamp}@furlpay-bot.test`;

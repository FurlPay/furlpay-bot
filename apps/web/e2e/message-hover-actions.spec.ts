...
test("message hover shows Reply and Copy; reply links to parent", async ({ page }, testInfo) => {
  ...
  await signup(page, `hover-actions-${stamp}@furlpay-bot.test`, "password12", "Hover Actions");
...
test("reply preview jumps to parent outside the loaded page", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `hover-page-${stamp}@furlpay-bot.test`, "password12", "Hover Page");
  await completeOnboarding(page);

package com.furlpaybot.notifications
...
class FurlPayBotNotificationService : Service() {
  ...
  private fun post(run: RunRecord, copy: NotificationCopy) {
    ...
    val notification = builder(copy.channel)
      .setSmallIcon(R.drawable.ic_furlpayBot_notification)
      .setContentTitle(copy.title)
      .setContentText(copy.body)
      .setStyle(Notification.BigTextStyle().bigText(copy.body))
      .setContentIntent(openApp(run))
      .addExtras(Bundle().apply {
        putString("furlpay-bot.spaceId", run.spaceId)
        putString("furlpay-bot.botId", run.botId)
        putString("furlpay-bot.threadId", run.threadId)
      })
      .setAutoCancel(true)
  ...
  private fun liveStatusIcon(run: RunRecord, avatarStyle: String): Icon {
    if (avatarStyle != "organic") {
      return Icon.createWithResource(this, R.drawable.ic_furlpayBot_notification)
    }
  ...
  private fun openApp(run: RunRecord? = null): PendingIntent {
    val intent = if (run == null) {
      packageManager.getLaunchIntentForPackage(packageName) ?: Intent(Intent.ACTION_VIEW, Uri.parse("furlpay-bot://"))
    } else {
      val destination = if (run.groupId != null) {
        "furlpay-bot://group-thread?groupId=${Uri.encode(run.groupId)}&name=${Uri.encode(run.groupName.orEmpty())}&spaceId=${Uri.encode(run.spaceId)}"
      } else {
        "furlpay-bot://thread?botId=${Uri.encode(run.botId)}&name=${Uri.encode(run.botName)}&spaceId=${Uri.encode(run.spaceId)}"
      }
      Intent(
        Intent.ACTION_VIEW,
  ...
  companion object {
    ...
    private const val ACTION_THREAD_CHANGED = "com.furlpaybot.notifications.THREAD_CHANGED"
    private const val PROMOTED_ONGOING_EXTRA = "android.requestPromotedOngoing"
    private const val POLL_INTERVAL_MS = 8_000L
    private const val STATE_PREFERENCES = "com.furlpaybot.notification_state"
    ...
    fun start(context: Context) {
      val intent = Intent(context, FurlPayBotNotificationService::class.java)
      context.startService(intent)
    }

    fun stop(context: Context) {
      context.stopService(Intent(context, FurlPayBotNotificationService::class.java))
    }
    ...
    fun setOpenThread(context: Context, botId: String?, threadId: String?) {
      ...
      context.startService(
        Intent(context, FurlPayBotNotificationService::class.java).setAction(ACTION_THREAD_CHANGED),
      )
    }
...
private object Channels {
  const val LIVE = "furlpayBot_live"
  const val MESSAGES = "furlpayBot_messages"
  const val SCHEDULED = "furlpayBot_scheduled"
  const val ATTENTION = "furlpayBot_attention"
}
...
private fun rpc(
  ...
  return try {
    ...
    connection.setRequestProperty("Origin", "furlpay-bot://")
    connection.setRequestProperty("Authorization", "Bearer $token")
    if (spaceId.isNotBlank()) {
      connection.setRequestProperty("x-furlpay-bot-space-id", spaceId)
    }

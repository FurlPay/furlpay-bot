...
class FurlPayBotNotificationsModule : Module() {
  override fun definition() = ModuleDefinition {
    Name("FurlPayBotNotifications")
    ...
    AsyncFunction("setSettings") { value: Map<String, Boolean>, endpoint: String, token: String, spaceId: String ->
      ...
      if (storage.settings.liveConnection && endpoint.isNotBlank() && token.isNotBlank()) {
        FurlPayBotNotificationService.start(context)
      } else {
        FurlPayBotNotificationService.stop(context)
      }
    }
    ...
    AsyncFunction("stop") { clearSession: Boolean ->
      val context = context()
      if (clearSession) {
        FurlPayBotNotificationService.clearSession(context)
        val storage = NotificationStorage(context)
        storage.token = ""
        storage.spaceId = ""
      } else {
    ...
    AsyncFunction("setOpenThread") { botId: String?, threadId: String? ->
      FurlPayBotNotificationService.setOpenThread(context(), botId, threadId)
    }

...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://furlpay-bot.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://furlpay-bot.example.com:8443",
    });
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://furlpay-bot.example.com")).toBe("furlpay-bot.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://furlpay-bot.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a FurlPay Bot /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not FurlPay Bot", async () => {
    const fetchImpl = vi.fn(

...
const ENDPOINT_KEY = "furlpay-bot.api_base";
const SPACE_KEY = "furlpay-bot.space_id";
const SPACE_ROLLBACK_KEY = "furlpay-bot.space_rollback";
...
export async function changePassword(currentPassword: string, newPassword: string): Promise<void> {
  const response = await fetch(`${currentApiBase()}/api/auth/change-password`, {
    method: "POST",
    headers: { "content-type": "application/json", origin: "furlpay-bot://", ...(await authHeaders()) },
...
export async function signOut() {
  ...
  await fetch(`${currentApiBase()}/api/auth/sign-out`, {
    method: "POST",
    headers: { "content-type": "application/json", origin: "furlpay-bot://", ...headers },
  }).catch(() => undefined);
  const sessionCleared = await clearSessionToken();

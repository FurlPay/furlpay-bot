...
const nativeNotifications =
  Platform.OS === "android"
    ? requireNativeModule<NativeNotifications>("FurlPayBotNotifications")
    : null;
...
export function notificationTargetsThread(
  ...
  const dataThreadId = data.threadId ?? data["furlpay-bot.threadId"];
  if (target.threadId && dataThreadId) return dataThreadId === target.threadId;
  return Boolean(
    target.botId && (data.botId === target.botId || data["furlpay-bot.botId"] === target.botId),
  );
}

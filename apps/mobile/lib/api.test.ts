...
describe("mobile API authentication", () => {
  ...
  it("persists a successful sign-in token and sends the native origin", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        headers: { "content-type": "application/json", origin: "furlpay-bot://" },
        body: JSON.stringify({ email: "ada@example.com", password: "correct horse" }),
      }),
    );
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith("furlpay-bot.session_token", "session-token");
  ...
  it("creates an account and persists its session token", async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith("furlpay-bot.session_token", "signup-token");
  });
  ...
  it("loads password recovery capability and requests a server-approved redirect", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({ passwordReset: true, resetUrl: "https://furlpay-bot.test/reset-password" }),
      )
    ...
    await expect(passwordResetCapabilities()).resolves.toEqual({
      passwordReset: true,
      resetUrl: "https://furlpay-bot.test/reset-password",
    });
    await requestPasswordReset("ada@example.test", "https://furlpay-bot.test/reset-password");
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        body: JSON.stringify({
          ...
          redirectTo: "https://furlpay-bot.test/reset-password",
        }),
      }),
    );
  });
  ...
  it("does not send a password or bearer token to a persisted public HTTP server", async () => {
    vi.mocked(SecureStore.getItemAsync).mockImplementation(async (key) => {
      if (key === "furlpay-bot.api_base") return "http://app.example.test";
      if (key === "furlpay-bot.session_token") return "session-token";
      return null;
    });
    const fetchMock = vi.fn(async () => jsonResponse({ status: true }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("starts notifications only after the inbox selects the default space", async () => {
    vi.mocked(SecureStore.getItemAsync).mockImplementation(async (key) =>
      key === "furlpay-bot.session_token" ? "session-token" : null,
    );
  ...
  it("clears the local session even when the sign-out request fails", async () => {
    ...
    expect(SecureStore.deleteItemAsync).toHaveBeenCalledWith("furlpay-bot.session_token");
  });
  ...
  it("shares the selected space with direct API requests", async () => {
    ...
    await expect(authHeaders()).resolves.toEqual({
      ...
      "x-furlpay-bot-space-id": "space-support",
    });
  });
  ...
  it("does not switch spaces when the selection cannot be persisted", async () => {
    ...
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key) => {
      if (key === "furlpay-bot.space_id") throw new Error("device locked");
    });
  ...
  it("does not switch spaces when stale recovery cannot be cleared", async () => {
    ...
    vi.mocked(SecureStore.deleteItemAsync).mockImplementation(async (key) => {
      if (key === "furlpay-bot.space_rollback") throw new Error("device locked");
    });
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key, value) => {
      if (key === "furlpay-bot.space_rollback" && value === "") throw new Error("device locked");
    });

    await expect(selectSpace("space-social")).resolves.toBe(false);
    expect(SecureStore.setItemAsync).not.toHaveBeenCalledWith("furlpay-bot.space_id", "space-social");
    expect(selectedSpaceId()).toBe("space-support");
  ...
  it("clears server-specific session and space state when the API endpoint changes", async () => {
    ...
    expect(SecureStore.deleteItemAsync).toHaveBeenCalledWith("furlpay-bot.session_token");
    expect(SecureStore.deleteItemAsync).toHaveBeenCalledWith("furlpay-bot.space_id");
    await resetApiBase();
  });
  ...
  it("restores notifications to the selected space when endpoint rollback succeeds", async () => {
    ...
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key, value) => {
      if (key === "furlpay-bot.session_token" || (key === "furlpay-bot.space_id" && value === "")) {
  ...
  it("restores credentials when the new endpoint cannot be persisted", async () => {
    ...
    await selectSpace("space-support");
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key) => {
      if (key === "furlpay-bot.api_base") throw new Error("device locked");
    });
  ...
  it("restores a persisted space when its initial load failed", async () => {
    ...
    let spaceReads = 0;
    vi.mocked(SecureStore.getItemAsync).mockImplementation(async (key) => {
      if (key !== "furlpay-bot.space_id") return null;
      spaceReads += 1;
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith("furlpay-bot.space_id", "space-support");
  });
  ...
  it("keeps an invalidated empty session fail closed during credential rollback", async () => {
    ...
    await selectSpace("space-support");
    vi.mocked(SecureStore.deleteItemAsync).mockImplementation(async (key) => {
      if (key === "furlpay-bot.session_token") throw new Error("device locked");
    });
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key, value) => {
      if (key === "furlpay-bot.session_token" && value === "") throw new Error("device locked");
    });
  ...
  it("recovers the active space after rollback persistence fails", async () => {
    ...
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key, value) => {
      if (key === "furlpay-bot.api_base" || (key === "furlpay-bot.space_id" && value === "space-support")) {
        throw new Error("device locked");
      }
    ...
    expect(storage.get("furlpay-bot.space_rollback")).toBe(
    ...
    expect(storage.get("furlpay-bot.space_id")).toBe("space-support");
    expect(storage.has("furlpay-bot.space_rollback")).toBe(false);
  });

  it("does not recover a space on a different endpoint", async () => {
    const storage = new Map([
      ["furlpay-bot.api_base", "https://second-server.example"],
      [
        "furlpay-bot.space_rollback",
        JSON.stringify({ apiBase: "http://127.0.0.1:3100", spaceId: "space-support" }),
      ],
    ]);
    vi.mocked(SecureStore.getItemAsync).mockImplementation(async (key) => storage.get(key) ?? null);
  ...
  it("removes a malformed space rollback record", async () => {
    const storage = new Map([["furlpay-bot.space_rollback", "null"]]);
    vi.mocked(SecureStore.getItemAsync).mockImplementation(async (key) => storage.get(key) ?? null);
    vi.mocked(SecureStore.setItemAsync).mockImplementation(async (key, value) => {
      storage.set(key, value);
    });

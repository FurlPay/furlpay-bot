...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/furlpay-bot-notifications/android/src/main/java/com/furlpaybot/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "FurlPayBotNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "FurlPayBotNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("FurlPayBotNotificationService.clearSession(context)");
    ...
    expect(service).toContain('connection.setRequestProperty("x-furlpay-bot-space-id", spaceId)');
    ...
    expect(service).toContain('"furlpay-bot://group-thread?groupId=');
    expect(service).toMatch(/&spaceId=\$\{Uri\.encode\(run\.spaceId\)\}/);
    expect(service).toContain('putString("furlpay-bot.spaceId", run.spaceId)');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/furlpay-bot-notifications/android/src/main/java/com/furlpaybot/notifications/FurlPayBotNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("furlpay-bot.botId", run.botId)');

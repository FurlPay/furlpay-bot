...
import { blockedAuthPaths, createAuth } from "@furlpay-bot/auth";
import { signupPolicyFromEnv } from "@furlpay-bot/core";
import {
  createDb,
  createThreadEvents,
  type PrismaClient,
...
import { MarkdownMemoryStore } from "@furlpay-bot/memory";
...
export async function createApp(
  ...
  app.use("/rpc/*", async (c, next) => {
    ...
    const requestedSpaceId = c.req.header("x-furlpay-bot-space-id");
  ...
  mountVoiceHttpRoutes(app, { prisma, secrets }, async (c) => {
    ...
    return requireMembership(prisma, session.user.id, c.req.header("x-furlpay-bot-space-id")).catch(
      () => null,
    );
  });
...
function isTrustedOrigin(origin: string, env: AppEnv) {
  ...
  if (origin.startsWith("furlpay-bot://") || origin.startsWith("exp://")) return true;
  try {

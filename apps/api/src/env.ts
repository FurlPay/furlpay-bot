import { resolveDeploymentModel, resolveSandboxProvider } from "@furlpay-bot/adapters";
import {
  resolveAuthSecret,
...
export { resolveSandboxProvider } from "@furlpay-bot/adapters";
...
export function loadEnv(source: NodeJS.ProcessEnv = process.env): AppEnv {
  ...
  const updaterUrl = optional(source.FURLPAY_UPDATER_URL);
  const updaterToken = optional(source.FURLPAY_UPDATER_TOKEN);
  return {
    ...
    gitSha: optional(source.GIT_SHA) ?? optional(source.FURLPAY_GIT_SHA),
    updaterUrl,
    updaterToken,
    imageTag: optional(source.FURLPAY_IMAGE_TAG),
  };
}

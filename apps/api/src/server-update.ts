...
import { ServerUpdateRunSchema } from "@furlpay-bot/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `FURLPAY_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.FURLPAY_IMAGE_TAG?.trim() || null;

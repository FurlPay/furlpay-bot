...
Same as the README quick start: `.env` from `.env.example`, Postgres via Compose, `pnpm sandbox:build`, `pnpm dev`, then [http://127.0.0.1:5173](http://127.0.0.1:5173). Electron: `pnpm --filter @furlpay-bot/desktop dev` while that stack is up.

## Published images (no checkout)

Pull Postgres and `ghcr.io/furlpay/furlpay-bot/app` into any empty folder. No clone or image build.
...
```bash
export FURLPAY_DOWNLOAD_BASE=https://example.com/mirror/furlpay-bot/infra/compose
bash install-images.sh
```

Trailing slashes on `FURLPAY_DOWNLOAD_BASE` are trimmed; non-HTTPS bases are rejected. Downloads
...
`FURLPAY_DOWNLOAD_SKIP_EXISTING=1` and/or pass `--local`:

```bash
# after placing docker-compose.images.yml and .env.images.example locally
bash install-images.sh --local --prepare-only
# or
FURLPAY_DOWNLOAD_SKIP_EXISTING=1 bash install-images.sh --prepare-only
```
...
```bash
export FURLPAY_INSTALLER_URL=https://example.com/mirror/furlpay-bot/infra/compose/install-images.sh
mkdir -p furlpay-bot && cd furlpay-bot &&
curl -fsSLO "${FURLPAY_INSTALLER_URL}" &&
bash install-images.sh
```

Stage C (`docker compose pull`) uses `FURLPAY_IMAGE`, `FURLPAY_IMAGE_TAG`,
`FURLPAY_COMPUTER_IMAGE`, and `FURLPAY_COMPUTER_IMAGE_TAG` (defaults
`ghcr.io/furlpay/furlpay-bot/{app,computer}`). When GHCR is unreachable, override those
...
```env
FURLPAY_IMAGE=registry.example.com/mirror/furlpay/furlpay-bot/app
FURLPAY_IMAGE_TAG=edge
FURLPAY_COMPUTER_IMAGE=registry.example.com/mirror/furlpay/furlpay-bot/computer
FURLPAY_COMPUTER_IMAGE_TAG=edge
```
...
(from the app image, on the internal network only) and pulls `ghcr.io/furlpay/furlpay-bot/computer`.
...
`FURLPAY_IMAGE_TAG` and `FURLPAY_COMPUTER_IMAGE_TAG` to the same published multi-arch release tag
when one exists (see [Published images and tags](#published-images-and-tags)). Changing only
`FURLPAY_IMAGE_TAG` leaves the computer service on amd64-only `edge`. Do not assume `latest` is
published.
...
`FURLPAY_HOST` to its hostname (for example, `app.example.com`).
...
Compose runs Postgres, the sandbox supervisor (Docker socket), API, worker, and a Vite preview of the web app. Bot computers are sibling containers (`furlpay-bot/computer:local`) on separate per-bot networks; only the supervisor and screen proxy join each one. The API process does not get an unrestricted Docker socket; the supervisor owns the lifecycle.
...
appears on sign-in only when a transactional email provider is available. FurlPay Bot uses a
...
EMAIL_FROM=FurlPay Bot <no-reply@example.com>
```
...
STARTTLS. FurlPay Bot rejects configuration that disables TLS or certificate verification.
...
```env
FURLPAY_LOCAL_MODELS=qwen3:4b,llama3.1:8b
FURLPAY_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
FURLPAY_LOCAL_CONTEXT_WINDOW=32768
FURLPAY_LOCAL_MAX_TOKENS=4096
```

The loopback default is suitable when running FurlPay Bot from a source checkout. In Docker Compose,
...
leave FurlPay Bot through this URL. Leave `FURLPAY_LOCAL_MODELS` blank to disable the provider.
...
the exact model id from that server, and an optional API key. By default FurlPay Bot only allows
loopback, RFC1918, and `host.docker.internal` targets. To permit public hostnames, set
`FURLPAY_OPENAI_COMPAT_ALLOW_PUBLIC=1` in the deployment environment. Public hostnames must resolve
...
- **Published images** (`docker-compose.images.yml`) default to `SANDBOX_PROVIDER=docker` with a
  local supervisor and published `ghcr.io/furlpay/furlpay-bot/computer` image. No E2B account required.
...
- **E2B** runs bot computers away from the FurlPay Bot host and is a good choice for public or multi-user
  production deployments. FurlPay Bot checkpoints the portable workspace and browser-profile directory to
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally
  `BOX_API_URL`. FurlPay Bot always creates or resumes boxes with `noEnv: true`, keeps the portable
  workspace under `/home/user/furlpay-bot-home`, and refreshes a two-hour TTL. A Box currently exposes one
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `FURLPAY_HOST`, and the three public origins. Set `FURLPAY_DEPLOY_DIR` when the checkout is not at
   the supported Linux default, `/srv/furlpay-bot`. Use URL-safe random values for database credentials.
   If you enable the `updater` profile, also set a dedicated `FURLPAY_UPDATER_TOKEN` (at least 32
...
WAKEUP_DRIVER=graphile
DATA_DIR=/data
# Absolute path of this checkout as the Docker daemon sees it. /srv/furlpay-bot is the Linux default;
# set this explicitly for every other layout. See "The deploy directory must be one path" below.
FURLPAY_DEPLOY_DIR=/srv/furlpay-bot
FURLPAY_IMAGE_TAG=local
# Optional: required only with `--profile updater`.
# FURLPAY_UPDATER_TOKEN=replace-with-32-plus-character-updater-token
```
...
**Build, do not pull, for a first deployment.** `FURLPAY_IMAGE_TAG` ships as `local`, a tag no
...
`FURLPAY_IMAGE_TAG` to that release tag and running `pull` followed by `up -d --wait --pull never`.
...
`/usr/local/sbin/furlpay-bot-backup` and enable the supplied `furlpay-bot-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/furlpay-bot`, with mode
...
`FURLPAY_IMAGE_TAG` (or rebuild `local`) and `up -d --wait --pull never`.

The updater sidecar has its own image and tag so an update never recreates the process performing
it. Move it deliberately by setting `FURLPAY_UPDATER_IMAGE_TAG` to the full `sha-<commit>` tag, then
...
`GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @furlpay-bot/db migrate`, then restart API and worker.
...
| Image | Contents |
| --- | --- |
| `ghcr.io/furlpay/furlpay-bot/app` | api, worker, web, and sandbox supervisor — one image, multiple commands |
| `ghcr.io/furlpay/furlpay-bot/computer` | Linux desktop used as each bot computer |
| `ghcr.io/furlpay/furlpay-bot/updater` | the updater sidecar, plus the Docker CLI |
...
also pull the same app tags once `FURLPAY_IMAGE_TAG` is set to a published value.

If you deploy from your own fork, set `FURLPAY_IMAGE` and `FURLPAY_UPDATER_IMAGE` to your namespace —
...
`FURLPAY_IMAGE_TAG` and `FURLPAY_COMPUTER_IMAGE_TAG` to the same published release tag rather than
`edge`. Changing only `FURLPAY_IMAGE_TAG` leaves the computer service on amd64-only `edge`. Until a
...
content addressing can pin `FURLPAY_IMAGE` outside the automatic updater to a verified digest.
...
`FURLPAY_UPDATER_TOKEN` and explicitly start the profile:
...
`FURLPAY_UPDATER_TOKEN`. Operator CLI upgrades above do not need it; the sidecar is for automated
...
- *Official repository:* resolves the newest stable release and its source commit with
  ...
  outgoing tag in `FURLPAY_IMAGE_TAG_PREVIOUS`, explicitly pulls the new image, then runs
...
- *Fork (Advanced):* a fork has no published images, so the sidecar fast-forwards the checkout in
  `FURLPAY_DEPLOY_DIR` and runs `up -d --build`. This builds on the server and takes minutes rather
...
`FURLPAY_DEPLOY_DIR` is bind-mounted into the updater at the same path it is read from
(`${FURLPAY_DEPLOY_DIR}:${FURLPAY_DEPLOY_DIR}`), and that is load-bearing rather than tidy. Production
Compose defaults both sides to `/srv/furlpay-bot`; set the variable for any other layout. When the
updater runs `docker compose -p <project> --file $FURLPAY_DEPLOY_DIR/infra/compose/docker-compose.prod.yml up -d`,
...
`FURLPAY_COMPOSE_PROJECT_NAME`; the final fallback is `furlpay-bot-prod`. Without that propagation, a
...
- **Linux.** The daemon shares the host filesystem, so the checkout path is the answer:
  `/srv/furlpay-bot` is the default and supported production layout. Set `FURLPAY_DEPLOY_DIR` explicitly
  when the checkout is elsewhere.
- **Docker Desktop (Windows/macOS).** The daemon runs in a VM that mounts your drive somewhere else.
  On Windows, `C:` appears at `/run/desktop/mnt/host/c`, so a checkout at `C:\Users\you\furlpay-bot` is
  `FURLPAY_DEPLOY_DIR=/run/desktop/mnt/host/c/Users/you/furlpay-bot`. Host Git may use `core.autocrlf=true`; the updater ignores CR-only diffs so that does not block `/apply`. Verify the mount before deploying:

```bash
docker compose --env-file .env -f infra/compose/docker-compose.prod.yml \
  --profile updater run --rm updater git -C "$FURLPAY_DEPLOY_DIR" log --oneline -1
```
...
- The process environment carries only updater settings (`FURLPAY_UPDATER_TOKEN`, deploy path,
...
Enabling the `updater` profile requires `FURLPAY_UPDATER_TOKEN` to be a dedicated random value (at
...
## What “FurlPay Bot Cloud” still needs
...
10. Turn on `SIGNUP_ALLOWLIST` until you want open registration. There is no FurlPay Bot-managed model billing in version 1 — users bring keys.

Expo / desktop installers are clients of that origin (`EXPO_PUBLIC_API_URL`, `FURLPAY_WEB_URL`). They are not a Cloud control plane.
